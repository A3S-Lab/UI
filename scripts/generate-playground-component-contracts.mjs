...
const contracts = [
  ...
  active("ProductExpertAvatar", "capabilities", "icons", "expert-avatar-composition", ".product-expert-avatar[data-avatar-variant]", "distinguish configured experts at a glance through stable, composed portraits while relying on nearby text for identity and state", "deterministic palette, hair, facial-detail, and accessory composition across catalog rows, resource pickers, composer chips, and detail views", "all twelve seeded experts retain distinct stable variants across rerenders, decorative portraits remain hidden from assistive technology, and compact rendering never distorts their aspect ratio"),
  ...
  active("ProductTestLogViewer", "session", "session", "session-detail-conversation", ".product-session-execution .log-viewer", "read ordered test output with stream identity, severity filtering, and copy feedback without leaving the execution timeline", "the shared public Log Viewer fixture, deterministic test streams, severity controls, autoscroll state, copy intent, and bounded output presentation", "long output remains locally scrollable, stdout and stderr stay distinguishable without color alone, filtering preserves chronology, and copied output never claims hidden lines were included"),
