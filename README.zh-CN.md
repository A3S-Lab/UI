...
A3S UI �?A3S Office 中精炼的交互模式转变为可重用的语�?HTML。它结合�?Tailwind CSS v4、本机浏览器元素和小型普�?JavaScript 控制器，无需 React、Radix 或框架运行时�?

该系统现在公开�?116 个公共组件合约�?86 个通用组件涵盖控件、导航、内容、覆盖和产品结构，�?30 个组�?Harness 则对编码代理的任务、对话、执行、审查、证据、文件导航、知识管理、编辑、终端、日志和设备预览工作流程进行分组�?
...
请参�?[installation guide](https://a3s-lab.github.io/UI/installation.html) 了解拆分 CSS 导入、控制器�?JavaScript 导入和服务器渲染模板�?
...
|导航 |活动栏、面包屑导航、返回底部、选项卡、分页、侧边栏和目�?|
|覆盖|警报对话框、对话框、抽屉式菜单、下拉菜单、上下文菜单、弹出框、浮动面板、图像查看器、命令和工具提示 |
|反馈和状�?|警报、徽章、状态徽章、空、进度、骨架、旋转器、流文本�?Toast |
...
|线束|任务启动、任务工作区、代理编写器、代理成绩单、代理工作台、上下文选择器、消息状态、消息附件、消息引用、后续建议、任务计划、计划步骤、任务队列、批准请求、执行项目、检查点、工具调用、工具调用时间线、工具结果、更改审核、执行证据、工件卡、文件资源管理器、文件管理器、知识库�?D 代码图、代码编辑器、终端、日志查看器、和设备模拟器|
|公用事业 |滚动区域和主题切换器 |

可选的 Dockview 集成位于语义清单旁边，并添加�?Dock 工作区、网格视图、拆分视图和窗格视图，而无需更改 116 个组件的数量。它为本�?TypeScript、React �?Vue 重新导出完整�?Dockview 8.1 MIT 表面，然后添�?A3S 主题令牌、版本化布局持久性和 Edge Group 安全布局重置�?

集成�?[Form system](https://a3s-lab.github.io/UI/components/form-system/) 添加了确定�?Form Core、可视化设计器、受控渲染器、持久交互合约、云主机适配器、CLI、Web 组件、React Hook Form 集成和本�?Vue 可组合项。它的指南和交互式示例直接位�?A3S UI 组件目录中�?
...
- **形状和高�?* - 限制半径、边界和阴影，以保留信息密度�?
- **运动** �?简短、有目的的过渡，具有减少运动的支持�?
- **辅助功能** �?语义元素、显�?ARIA 状态、键盘交互、RTL 感知布局和浅�?深色主题�?
...
这些模式是独立可重用的，但它们的令牌和布局契约旨在组成文档编辑器、任务工作区和可观察性控制台。默认任务几何结构使�?248 像素导航区域�?60 像素阅读列�?20-380 像素可选检查器�?6 像素控件�?44 像素粗指针目标。响应式导航成为 768 像素以下的包含式抽屉；检查员变为低于 900 像素的覆盖层和低�?520 像素的底部抽屉�?
...
独立�?[Playground](https://a3s-lab.github.io/UI/playground.html) 是双语、任务优先的 A3S 产品应用程序，而不是组件展示或可停�?IDE。其规范�?`/playground` 路线系列将持久任务和项目会话、基�?TipTap 的编辑器、模型和工作量控制、工作空间参考、后续队列、权限审查、工具调用证据、Finder 风格的本地文件、代码和 Office/PDF 工作台、知识、内存、屏幕截图主导的灵感工作流程、扩展、自动化和生产设置结合在一个连贯的桌面和紧凑屏幕流程中。旧�?`/app` URL 仍然是路由别名�?Playground 位于文档章节层次结构之外�?

## 文档语言和版�?

该文档网站使用与 A3S Code 网站相同�?Rspress、React �?TypeScript 堆栈。默认语言为简体中文；每个发布的版本还提供英文文档。当前的每个组件�?Harness 集成指南都通过一个持久选项卡集来呈�?HTML、React �?Vue，其中包含框架正确的安装、所需的导入、突出显示的可复制示例以及公共合约具有行为的匹配挂钩或可组合项�?
...
|进口|目的|
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `@a3s-lab/ui` |完整的默�?A3S CSS 捆绑�?|
| `@a3s-lab/ui/base` |没有视觉样式的令牌、实用程序和结构组件 CSS |
| `@a3s-lab/ui/components/{name}.css` |一个组件的结构CSS |
| `@a3s-lab/ui/styles/a3s.css` |用于分割导入构建�?A3S 视觉基础 |
| `@a3s-lab/ui/runtime` |共享生命周期和控制器注册表|
| `@a3s-lab/ui/all` |共享运行时加上所有自动初始化控制器（图表 | 除外�?
| `@a3s-lab/ui/{controller}` |一�?JavaScript 控制器，例如 `app-shell`、`task-workspace`、`tabs`、`split-pane`、`code-editor` �?`device-simulator` |
| `@a3s-lab/ui/manifest` |所�?116 个公共组件的机器可读元数�?|
| `@a3s-lab/ui/components.json` | JSON 组件选择器、部件、操作、状态和测试选择�?|
| `@a3s-lab/ui/ai` | DOM 注释、发现、选择器和快照助手 |
| `@a3s-lab/ui/a3s-test` |准备运行的确定性工作流程示�?|
| `@a3s-lab/ui/a3s-test/selectors` |组件、部分、操作、就绪和状态选择器助�?|
| `@a3s-lab/ui/react` |可选的�?React 适配器和类型化控制器挂钩�?React 仍然是对等依�?|
| `@a3s-lab/ui/vue` |可选的�?Vue 适配器和类型化控制器可组合项�?Vue 仍然是对等依�?|
| `@a3s-lab/ui/dockview` |完整的本�?Dockview、Gridview、Splitview �?Paneview API 以及 A3S 主题和持久�?|
| `@a3s-lab/ui/dockview/react` |完整�?React Dockview API 以及布局和视图挂�?|
| `@a3s-lab/ui/dockview/vue` |完整�?Vue 3 Dockview API 以及匹配的可组合�?|
| `@a3s-lab/ui/dockview/css` | Dockview 基础 CSS �?A3S 浅色/深色主题变量 |
| `@a3s-lab/ui/form` |表单文档、编译器、设计器、渲染器、适配器和集成合约 |
| `@a3s-lab/ui/form/core` |独立于框架的编译、规则、验证、补丁、区域设置目录和无头状�?|
| `@a3s-lab/ui/form/react` | React Designer、Renderer、文件上传、签名、数据源和检查器控件 |
| `@a3s-lab/ui/form/react-hooks` | React Hook Form 兼容`useA3SForm`、解析器、字段数组、订阅和渲染器绑�?|
| `@a3s-lab/ui/form/vue` | Vue 3 设计器和渲染器适配�?|
| `@a3s-lab/ui/form/vue-hooks` | Native Vue `useA3SForm`、字段、字段数组、上下文、验证和渲染器可组合�?|
| `@a3s-lab/ui/form/web-component` |框架中立�?`<a3s-form-designer>` �?`<a3s-form-renderer>` 自定义元�?|
| `@a3s-lab/ui/form/cloud` |类型A3S Cloud主机适配器|
| `@a3s-lab/ui/form/a3s-ui.css` | A3S UI plus 产品界面的表单布局和交互样�?|
| `@a3s-lab/ui/templates/*` |用于服务器渲染应用程序的 Nunjucks �?Jinja 模板 |
...
React �?Vue 还公开�?`useA3SLocale`、`useA3STheme` �?`useA3SMotion`。这些钩子和可组合项与原�?HTML 遵循相同的根 `lang`、`dir`、`data-theme`、`.dark` 和媒体查询约定；翻译资源、持久性和服务器协商仍然由应用程序拥有�?

运行时还会在视觉视口内保持打开�?`[data-popover]` 曲面。下拉菜单、弹出窗口、选择和组合框共享碰撞翻转、可用高度受限、实时滚�?调整大小定位和逻辑 RTL 对齐�?
...
使用 `npm run test:e2e:a3s` 运行特定于组件和特定于路由的浏览器套件。报告的缺陷类保留为页面级A3S Test回归，包含操作状态断言、屏幕截图、控制台输出和页面错误证据。默认情况下，场景连续运行，因此状态预览保持确定性；仅当浏览器适配器有足够的隔离能力时才设置`A3S_TEST_MAX_PARALLEL`。该命令期望 `PATH` 上有 `a3s-test`；当本地适配器需要显式路径时，使�?`A3S_TEST_BIN`、`A3S_TEST_BROWSER_DRIVER` �?`A3S_TEST_BROWSER_EXECUTABLE`�?

发布合同目前�?116 个公共组件�?4 �?Playground 导出以及 20 �?Harness �?Playground 表面映射�?230 个实质�?PRD。检查的 ACL 库存和证据总数位于`product-requirements/README.md`�?`npm run test:e2e:a3s:check` 在浏览器运行之前拒绝丢失、重复、浅、陈旧或无效的映射；机器可读索引位于`product-requirements/`下�?

视觉检查使用具有特定于平台的桌面和紧凑基线�?Playwright。每个公共组件路由还具有组件根几何结构和状态契约以及浏览器诊断覆盖范围。设置`A3S_UI_VISUAL_CHROMIUM_EXECUTABLE`以重用系统Chromium安装，并在默认本地端口被占用时设置`A3S_UI_VISUAL_PORT`；这些检查故意不属于 CI 的一部分�?

## 血统和许可

A3S UI 建立在由 [Ronan Berder](https://github.com/hunvreus) 创建�?[Basecoat](https://github.com/hunvreus/basecoat) 的基础上，并保留其�?[shadcn/ui](https://ui.shadcn.com/) 视觉语言的语�?HTML 解释�?A3S 主题和工作台组件扩展�?A3S 产品的基础�?
