...
| Priority | P0 |
| Category | `forms` |
| Public route | `/en/components/textarea.html` |
...
Textarea captures one plain-text value whose line breaks are meaningful. It preserves native multiline editing while keeping large pastes, validation, and availability states contained. The component is justified only when this repeated job remains clearer and safer than raw native markup or an existing composition. Its product decision is **Keep**, so implementation must preserve that scope instead of expanding into a parallel product surface.

## Product boundary

Preserve native multiline editing, IME, selection, resize, and paste. Auto-grow is optional and bounded; counts and limits never displace the error message.

The host application continues to own domain data, authorization, transport, persistence, analytics, and irreversible side effects unless this contract explicitly names a local browser behavior. The component owns only the semantic root, documented parts, local interaction state, and integration events or methods listed below.

## States

- `empty` — Textarea states what is absent and why when known, then offers only a valid next action.
- `ready` — Textarea is stable, named, and ready for its primary reading or interaction job.
- `disabled` — Textarea remains understandable but cannot be changed; native disabled or read-only semantics must match the visual treatment.
- `invalid` — Textarea explains the unavailable or failed outcome without discarding prior context and exposes recovery only when recovery is valid.
- `readonly` — Textarea remains understandable but cannot be changed; native disabled or read-only semantics must match the visual treatment.

## Interaction contract

- Canonical root: `textarea.textarea` on `<textarea>`.
- Stable automation root: `[data-a3s-components~="textarea"]`.
...
Component-specific adversarial coverage: Huge paste, multiline IME, min/max height, long word, character limit, invalid/disabled/read-only, virtual keyboard, and draft persistence.

## Accessibility

The canonical root uses `<textarea>` semantics and exposes 0 named parts. State must be available through native properties, text, or documented ARIA rather than color, motion, or icon shape alone. Keyboard users must be able to complete `fill`, `focus`, `type` without a precise pointer. Focus indicators use the shared focus contract, reduced-motion preferences are respected, and names remain meaningful in both supported locales.
...
- The public root matches `textarea.textarea` and is annotated by the runtime as `[data-a3s-components~="textarea"]`.
- Every documented state above has an independent specimen cloned from the live public root; no fixture may claim mutually exclusive states on one instance.
- The state acceptance matrix opens at `.a3s-component-state-matrix[data-open][data-component=textarea]`, preserves hidden roots in the DOM contract, and keeps product dismiss/focus restore for interactive users (contract runs reload after matrix evidence because CDP cannot dismiss the overlay).
- Pointer and keyboard paths produce the same outcome for every applicable action.
- The public-root live preview uses the same public assets and contract as a consumer integration.
- HTML, React, and Vue examples remain in the page's integrated code panel and preserve the same semantic root, states, events, and methods.
- Light, dark, LTR, RTL, desktop, and compact layouts preserve reading order, visible focus, and recovery.
- The adversarial cases are treated as release requirements: Huge paste, multiline IME, min/max height, long word, character limit, invalid/disabled/read-only, virtual keyboard, and draft persistence.
- Console and page-error evidence are empty for the deterministic acceptance path.

## A3S Test mapping

- Behavioral regression: `components-actions-forms.acl`, scenario `textarea`.
- Cross-framework, keyboard, responsive, theme, direction, and visual contract: `tests/e2e/component-contracts.acl`, scenario `component-contract-textarea`.
- Route under test: `http://127.0.0.1:4178/UI/en/components/textarea.html`.
- Stable root target: `[data-a3s-components~="textarea"]` inside `.a3s-preview[data-preview-component=textarea][data-preview-integration=complete]`.
- State-matrix screenshot: `components/contracts/textarea-states.png`.
- Per-state evidence selectors:
  - `empty`: `.a3s-component-state-matrix[data-open][data-component=textarea] [data-state-specimen=empty]:has([data-a3s-components~='textarea'][data-a3s-state~='empty'])`
  - `ready`: `.a3s-component-state-matrix[data-open][data-component=textarea] [data-state-specimen=ready]:has([data-a3s-components~='textarea'][data-a3s-state~='ready'])`
  - `disabled`: `.a3s-component-state-matrix[data-open][data-component=textarea] [data-state-specimen=disabled]:has([data-a3s-components~='textarea'][data-a3s-state~='disabled']:is([disabled],[aria-disabled=true],:has([disabled]),:has([aria-disabled=true])))`
  - `invalid`: `.a3s-component-state-matrix[data-open][data-component=textarea] [data-state-specimen=invalid]:has([data-a3s-components~='textarea'][data-a3s-state~='invalid']:is([aria-invalid=true],[data-validation-state=invalid],:has([aria-invalid=true])))`
  - `readonly`: `.a3s-component-state-matrix[data-open][data-component=textarea] [data-state-specimen=readonly]:has([data-a3s-components~='textarea'][data-a3s-state~='readonly']:is([readonly],[aria-readonly=true],[data-readonly],:has([readonly])))`
