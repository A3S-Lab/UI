...
Button Group organizes related actions into one visual and keyboard-navigation unit for adjacent, segmented, or menu-backed controls. The component is justified only when this repeated job remains clearer and safer than raw native markup or an existing composition. Its product decision is **Compose**, so implementation must preserve that scope instead of expanding into a parallel product surface.
...
- Named parts: `action` (`:scope > button, :scope > a, :scope > .btn`).
