...
Input captures a single line of text or a structured value and reduces mistakes through a persistent label, format guidance, and timely feedback. The component is justified only when this repeated job remains clearer and safer than raw native markup or an existing composition. Its product decision is **Keep**, so implementation must preserve that scope instead of expanding into a parallel product surface.

## Product boundary

Style native input behavior without flattening type-specific affordances, autofill, password managers, IME composition, or browser validation.
...
Component-specific adversarial coverage: Autofill colors, Chinese IME, password reveal, search clear, long unbroken value, 200% zoom, invalid/disabled/read-only, and mobile keyboard types.
...
- The adversarial cases are treated as release requirements: Autofill colors, Chinese IME, password reveal, search clear, long unbroken value, 200% zoom, invalid/disabled/read-only, and mobile keyboard types.
...
- Per-state evidence selectors:
  - `ready`: `.a3s-component-state-matrix[open][data-component=input] [data-state-specimen=ready]:has([data-a3s-components~='input'][data-a3s-state~='ready'])`
  - `disabled`: `.a3s-component-state-matrix[open][data-component=input] [data-state-specimen=disabled]:has([data-a3s-components~='input'][data-a3s-state~='disabled']:is([disabled],[aria-disabled=true],:has([disabled]),:has([aria-disabled=true])))`
  - `invalid`: `.a3s-component-state-matrix[open][data-component=input] [data-state-specimen=invalid]:has([data-a3s-components~='input'][data-a3s-state~='invalid']:is([aria-invalid=true],[data-validation-state=invalid],:has([aria-invalid=true])))`
  - `readonly`: `.a3s-component-state-matrix[open][data-component=input] [data-state-specimen=readonly]:has([data-a3s-components~='input'][data-a3s-state~='readonly']:is([readonly],[aria-readonly=true],[data-readonly],:has([readonly])))`
