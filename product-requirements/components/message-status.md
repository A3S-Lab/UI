...
- The state acceptance matrix opens at `.a3s-component-state-matrix[open][data-component=message-status]`, preserves hidden roots in the DOM contract, and exposes a close control that restores focus to its trigger for real keyboard and pointer users.
