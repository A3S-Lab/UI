...
| Route | `/playground/memory.html` |
| Stable selector | `[data-product-application][data-view=memory] [data-product-surface=memory]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-memory` |

## User problem

People evaluating the composition need to review durable memories, approve candidate changes, inspect provenance, and attach relevant context to a task.
...
The stable acceptance root is `[data-product-application][data-view=memory] [data-product-surface=memory]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=memory] [data-product-surface=memory]` after hydration.
