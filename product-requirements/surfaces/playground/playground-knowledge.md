...
| Route | `/playground/resources/knowledge.html` |
| Stable selector | `[data-product-application][data-view=resources] [data-product-surface=knowledge]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-knowledge` |

## User problem

People evaluating the composition need to manage knowledge bases and sources, understand ingestion health, and start work from selected durable context.
...
The stable acceptance root is `[data-product-application][data-view=resources] [data-product-surface=knowledge]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=resources] [data-product-surface=knowledge]` after hydration.
...
- Scenario: `playground-route-knowledge`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/resources/knowledge.html`.
- Stable target: `[data-product-application][data-view=resources] [data-product-surface=knowledge]`.
