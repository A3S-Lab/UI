# Seeded Session Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `playground` |
| Route | `/playground/sessions/fix-session-recovery.html` |
| Stable selector | `[data-product-application][data-view=session] [data-product-surface=session]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-session` |

## User problem

People evaluating the composition need to inspect a deterministic conversation, execution history, files, graph, and device preview in one task-first session.
...
The stable acceptance root is `[data-product-application][data-view=session] [data-product-surface=session]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=session] [data-product-surface=session]` after hydration.
...
- Scenario: `playground-route-session`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/sessions/fix-session-recovery.html`.
- Stable target: `[data-product-application][data-view=session] [data-product-surface=session]`.
