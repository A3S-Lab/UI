...
| Route | `/playground/assistant.html` |
| Stable selector | `[data-product-application][data-view=assistant] [data-product-surface=assistant]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-assistant` |

## User problem

People evaluating the composition need to start focused work with an assistant configuration while retaining access to files and model settings.
...
The stable acceptance root is `[data-product-application][data-view=assistant] [data-product-surface=assistant]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=assistant] [data-product-surface=assistant]` after hydration.
...
- Scenario: `playground-route-assistant`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/assistant.html`.
- Stable target: `[data-product-application][data-view=assistant] [data-product-surface=assistant]`.
