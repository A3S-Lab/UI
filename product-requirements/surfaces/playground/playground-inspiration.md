...
| Route | `/playground/resources/inspiration.html` |
| Stable selector | `[data-product-application][data-view=resources] [data-product-surface=inspiration]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-inspiration` |

## User problem

People evaluating the composition need to browse useful fragments, inspect their provenance, and deliberately convert one into structured work.
...
The stable acceptance root is `[data-product-application][data-view=resources] [data-product-surface=inspiration]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=resources] [data-product-surface=inspiration]` after hydration.
