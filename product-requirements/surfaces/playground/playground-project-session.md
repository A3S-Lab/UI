# Project Session Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `playground` |
| Route | `/playground/projects/a3s-ui-experience/sessions/release-readiness.html` |
| Stable selector | `[data-product-application][data-view=project-session] [data-product-surface=project-session]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-project-session` |

## User problem

People evaluating the composition need to review one project-bound session and its execution evidence without turning the session into a generic editor.
...
The stable acceptance root is `[data-product-application][data-view=project-session] [data-product-surface=project-session]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=project-session] [data-product-surface=project-session]` after hydration.
...
- Scenario: `playground-route-project-session`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/projects/a3s-ui-experience/sessions/release-readiness.html`.
- Stable target: `[data-product-application][data-view=project-session] [data-product-surface=project-session]`.
