...
| Route | `/playground/capabilities.html` |
| Stable selector | `[data-product-application][data-view=catalog] [data-product-surface=catalog]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-capabilities` |

## User problem

People evaluating the composition need to discover, inspect, configure, and remove assistants, skills, and connectors with explicit lifecycle and permission boundaries.
...
The stable acceptance root is `[data-product-application][data-view=catalog] [data-product-surface=catalog]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=catalog] [data-product-surface=catalog]` after hydration.
...
- Scenario: `playground-route-capabilities`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/capabilities.html`.
- Stable target: `[data-product-application][data-view=catalog] [data-product-surface=catalog]`.
