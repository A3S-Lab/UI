# Extensions Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `playground` |
| Route | `/playground/extensions.html` |
| Stable selector | `[data-product-application][data-view=marketplace] [data-product-surface=marketplace]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-extensions` |

## User problem

People evaluating the composition need to inspect extension capability, source, permission, installation, and failure states before changing local availability.
...
The stable acceptance root is `[data-product-application][data-view=marketplace] [data-product-surface=marketplace]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=marketplace] [data-product-surface=marketplace]` after hydration.
...
- Scenario: `playground-route-extensions`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/extensions.html`.
- Stable target: `[data-product-application][data-view=marketplace] [data-product-surface=marketplace]`.
