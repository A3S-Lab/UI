# Automations Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `playground` |
| Route | `/playground/automations.html` |
| Stable selector | `[data-product-application][data-view=automation] [data-product-surface=automation]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-automations` |

## User problem

People evaluating the composition need to create, inspect, pause, run, and recover scheduled work while keeping schedule and run history distinguishable.
...
The stable acceptance root is `[data-product-application][data-view=automation] [data-product-surface=automation]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=automation] [data-product-surface=automation]` after hydration.
...
- Scenario: `playground-route-automations`.
- Preview URL: `http://127.0.0.1:4178/UI/playground/automations.html`.
- Stable target: `[data-product-application][data-view=automation] [data-product-surface=automation]`.
