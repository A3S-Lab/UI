# New Task Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `playground` |
| Route | `/playground.html` |
| Stable selector | `[data-product-application][data-view=start] [data-product-surface=start]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-start` |

## User problem

People evaluating the composition need to compose a task with workspace, model, effort, capability, and resource context before submission.
...
The stable acceptance root is `[data-product-application][data-view=start] [data-product-surface=start]`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=start] [data-product-surface=start]` after hydration.
...
- Scenario: `playground-route-start`.
- Preview URL: `http://127.0.0.1:4178/UI/playground.html`.
- Stable target: `[data-product-application][data-view=start] [data-product-surface=start]`.
