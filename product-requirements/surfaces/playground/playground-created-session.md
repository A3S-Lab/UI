# Current Session Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `playground` |
| Route | `/playground/sessions/current.html` |
| Stable selector | `[data-product-application][data-view=created-session] [data-product-surface=session]` |
| A3S Test suite | `tests/e2e/playground-route-contracts.acl` |
| A3S Test scenario | `playground-route-created-session` |

## User problem

People evaluating the composition need to continue a locally created task, manage follow-up work, and recover honestly when no persisted session exists.
...
- The route resolves directly on first load and exposes `[data-product-application][data-view=created-session] [data-product-surface=session]` after hydration.
