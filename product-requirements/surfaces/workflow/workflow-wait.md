# Wait Until Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `workflow` |
| Route | `/components/form-system/a3s-flow/wait.html` |
| Stable selector | `.a3s-form-workflow-node-preview[data-node-type='flow.wait']` |
| A3S Test suite | `tests/e2e/workflow-surface-contracts.acl` |
| A3S Test scenario | `workflow-node-wait` |

## User problem

Workflow authors need to pause a durable run until a validated absolute UTC deadline or saved-state expression without editing an untyped configuration blob or guessing which ports are available.
...
The stable acceptance root is `.a3s-form-workflow-node-preview[data-node-type='flow.wait']`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.

Workflow-specific acceptance: Edit and apply the durable resume-time field path, reject an empty expression path, then restore the default saved-state deadline before running the wait node.
...
- The route resolves directly on first load and exposes `.a3s-form-workflow-node-preview[data-node-type='flow.wait']` after hydration.
...
- Component-specific edit, rejection, recovery, panel disclosure, focus return, running, and successful-result transitions are deterministic where this surface owns them. The exact workflow proof is: Edit and apply the durable resume-time field path, reject an empty expression path, then restore the default saved-state deadline before running the wait node.
