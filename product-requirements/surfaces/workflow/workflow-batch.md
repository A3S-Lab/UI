# Run Step Batch Product Requirements

| Field | Contract |
| --- | --- |
| Surface group | `workflow` |
| Route | `/components/form-system/a3s-flow/batch.html` |
| Stable selector | `.a3s-form-workflow-node-preview[data-node-type='flow.batch']` |
| A3S Test suite | `tests/e2e/workflow-surface-contracts.acl` |
| A3S Test scenario | `workflow-node-batch` |

## User problem

Workflow authors need to configure a non-empty ordered batch whose member identity remains durable across resume without editing an untyped configuration blob or guessing which ports are available.
...
The stable acceptance root is `.a3s-form-workflow-node-preview[data-node-type='flow.batch']`. Pointer and keyboard users must reach the same primary actions. Keyboard focus enters through named controls, activation uses native Enter or Space behavior, Escape or a repeated toggle closes transient layers where applicable, and focus returns to the control that opened the layer. Resizing or dragging always has a keyboard-accessible preset or command.

Workflow-specific acceptance: Edit and apply the first durable batch member ID, reject an empty member ID in-place, expand the invalid member, and recover the default ordered member without losing the batch node.
...
- The route resolves directly on first load and exposes `.a3s-form-workflow-node-preview[data-node-type='flow.batch']` after hydration.
...
- Component-specific edit, rejection, recovery, panel disclosure, focus return, running, and successful-result transitions are deterministic where this surface owns them. The exact workflow proof is: Edit and apply the first durable batch member ID, reject an empty member ID in-place, expand the invalid member, and recover the default ordered member without losing the batch node.
...
- Scenario: `workflow-node-batch`.
- Preview URL: `http://127.0.0.1:4178/UI/components/form-system/a3s-flow/batch.html`.
- Stable target: `.a3s-form-workflow-node-preview[data-node-type='flow.batch']`.
- Evidence: desktop screenshot, compact screenshot, interactive accessibility tree, console log, and page-error log. An additional invalid-state screenshot is required at `workflow/contracts/workflow-batch-invalid.png`. Harness and Workflow interaction scenarios additionally prove their component-specific keyboard state transitions, invalid-state focus, reset recovery, disclosure focus return, and running-to-result transition.
