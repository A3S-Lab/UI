...
The component is exercised at `.product-project-plan` by `tests/e2e/product-project-pages.acl#project-plan-page`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `project-plan-page` explicitly targets `.product-project-plan` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-project-pages.acl#project-plan-page`, with stable target `.product-project-plan`.
