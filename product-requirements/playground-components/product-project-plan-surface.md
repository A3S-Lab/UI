# Product Project Plan Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductProjectPlanSurface` |
| Source | `site/theme/components/playground/ProductProjectPlanSurface.tsx` |
...
| Behavior assertion | `plan-surface` |

## User problem

People using the composed application need to scan grouped plan work, collapse status groups, complete items, and add a task in the right state without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Project Plan Surface owns plan grouping, task state and metadata, collapse controls, completion intent, inline creation, empty-group recovery, and display-option response. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Project Plan Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-project-plan` by `tests/e2e/product-project-pages.acl#project-plan-page`. The uniquely assigned `plan-surface` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The export inventory contains exactly one `ProductProjectPlanSurface` declaration in `site/theme/components/playground/ProductProjectPlanSurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `project-plan-page` explicitly targets `.product-project-plan`; its unique `plan-surface` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- Behavior evidence: `tests/e2e/product-project-pages.acl#project-plan-page`, assertion `plan-surface`, with stable target `.product-project-plan`.
