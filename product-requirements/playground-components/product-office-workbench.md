...
The component is exercised at `[data-office-load-state=ready]` by `tests/e2e/product-workspace-capabilities.acl#all-file-workbenches`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `all-file-workbenches` explicitly targets `[data-office-load-state=ready]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#all-file-workbenches`, with stable target `[data-office-load-state=ready]`.
