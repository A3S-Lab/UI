# Product Extension Host Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductExtensionHost` |
| Source | `site/theme/components/playground/ProductExtensionHost.tsx` |
...
| Behavioral scenario | `extension-host-boundary` |
| Behavior assertion | `isolated-host` |

## User problem

People using the composed application need to open an installed extension inside an explicitly isolated host and review proposed task context before accepting it without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Extension Host owns host isolation copy, reload states, bounded proposal preview, accept intent, and return to marketplace context. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Extension Host may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-product-surface=extension-host]` by `tests/e2e/product-workspace-capabilities.acl#extension-host-boundary`. The uniquely assigned `isolated-host` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-product-surface=extension-host]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Extension Host must prove reload failure remains inside the host, proposed content cannot mutate the task before acceptance, and the surface never implies arbitrary extension code runs with product authority; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductExtensionHost` declaration in `site/theme/components/playground/ProductExtensionHost.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `extension-host-boundary` explicitly targets `[data-product-surface=extension-host]`; its unique `isolated-host` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Extension Host owns host isolation copy, reload states, bounded proposal preview, accept intent, and return to marketplace context. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Extension Host must prove reload failure remains inside the host, proposed content cannot mutate the task before acceptance, and the surface never implies arbitrary extension code runs with product authority; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#extension-host-boundary`, assertion `isolated-host`, with stable target `[data-product-surface=extension-host]`.
