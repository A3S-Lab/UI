...
The component is exercised at `[data-product-surface=extension-host]` by `tests/e2e/product-workspace-capabilities.acl#extension-host-boundary`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `extension-host-boundary` explicitly targets `[data-product-surface=extension-host]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#extension-host-boundary`, with stable target `[data-product-surface=extension-host]`.
