...
| Behavior assertion | `account-menu` |
...
- The real interaction path `account-menu-and-exit` explicitly targets `.product-account-menu[role=menu]`; its unique `account-menu` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#account-menu-and-exit`, assertion `account-menu`, with stable target `.product-account-menu[role=menu]`.
