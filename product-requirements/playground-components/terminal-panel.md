# Terminal Panel Product Requirements
...
| Boundary scenario | `playground-legacy-terminal-panel` |

## User problem

Maintainers need to know that Terminal Panel belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Terminal Panel may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is a dock panel from the retired workspace topology; command evidence now stays inside chronological session execution.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-terminal-panel` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Terminal Panel must prove no current route renders an always-on generic terminal or confuses deterministic command previews with an interactive shell; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `TerminalPanel` declaration in `site/theme/components/playground/WorkspacePanels.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-terminal-panel` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Terminal Panel may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is a dock panel from the retired workspace topology; command evidence now stays inside chronological session execution.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Terminal Panel must prove no current route renders an always-on generic terminal or confuses deterministic command previews with an interactive shell; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-terminal-panel`, plus the generator's active-import rejection.
