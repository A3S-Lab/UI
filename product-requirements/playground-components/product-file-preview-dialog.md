# Product File Preview Dialog Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductFilePreviewDialog` |
| Source | `site/theme/components/playground/ProductFilePreviewDialog.tsx` |
...
| Behavioral scenario | `files-icon-identity` |
| Behavior assertion | `preview-dialog-boundary` |

## User problem

People using the composed application need to inspect one selected file in a movable or fullscreen preview before opening, renaming, downloading, or attaching it without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product File Preview Dialog owns modal preview geometry, drag and fullscreen state, file metadata, task handoff actions, Escape cancellation, and exact trigger focus restoration. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product File Preview Dialog may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-file-preview-dialog]` by `tests/e2e/product-resource-surfaces.acl#files-icon-identity`. The uniquely assigned `preview-dialog-boundary` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-file-preview-dialog]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product File Preview Dialog must prove the preview stays inside the viewport, fullscreen retains a visible close action, unsupported content remains inert text, and closing never loses the selected file; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductFilePreviewDialog` declaration in `site/theme/components/playground/ProductFilePreviewDialog.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `files-icon-identity` explicitly targets `[data-file-preview-dialog]`; its unique `preview-dialog-boundary` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product File Preview Dialog owns modal preview geometry, drag and fullscreen state, file metadata, task handoff actions, Escape cancellation, and exact trigger focus restoration. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product File Preview Dialog must prove the preview stays inside the viewport, fullscreen retains a visible close action, unsupported content remains inert text, and closing never loses the selected file; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-resource-surfaces.acl#files-icon-identity`, assertion `preview-dialog-boundary`, with stable target `[data-file-preview-dialog]`.
