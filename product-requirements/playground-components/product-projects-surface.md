# Product Projects Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductProjectsSurface` |
| Source | `site/theme/components/playground/ProductTaskSurfaces.tsx` |
| Classification | `active` |
| Canonical route | `/playground/projects.html` |
| Route surface | `[data-product-surface=projects]` |
| Route evidence | `tests/e2e/playground-route-contracts.acl#playground-route-projects` |
| Behavioral suite | `tests/e2e/product-project-pages.acl` |
| Behavioral scenario | `project-list-page` |
| Behavior assertion | `ready` |

## User problem

People using the composed application need to scan available projects by status and recent activity and enter the correct workspace confidently without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Projects Surface owns project page hierarchy, search, deterministic project cards or rows, status and activity copy, empty filtering, and project-link navigation. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Projects Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-product-surface=projects]` by `tests/e2e/product-project-pages.acl#project-list-page`. The uniquely assigned `ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-product-surface=projects]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Projects Surface must prove search no-results retain the query, status is not color-only, long project names remain distinguishable, and the first project link works on direct page load; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductProjectsSurface` declaration in `site/theme/components/playground/ProductTaskSurfaces.tsx` and exactly one matching PRD and coverage record.
- The canonical route `/playground/projects.html` loads directly and passes `playground-route-projects` with desktop and phone screenshots, accessibility evidence, console capture, and page-error capture.
- The real interaction path `project-list-page` explicitly targets `[data-product-surface=projects]`; its unique `ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Projects Surface owns project page hierarchy, search, deterministic project cards or rows, status and activity copy, empty filtering, and project-link navigation. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Projects Surface must prove search no-results retain the query, status is not color-only, long project names remain distinguishable, and the first project link works on direct page load; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
- Console and page-error evidence contain no unexpected runtime failures on the deterministic acceptance path.

## A3S Test mapping

- Direct route evidence: `tests/e2e/playground-route-contracts.acl#playground-route-projects` at `http://127.0.0.1:4178/UI/playground/projects.html`.
- Behavior evidence: `tests/e2e/product-project-pages.acl#project-list-page`, assertion `ready`, with stable target `[data-product-surface=projects]`.
