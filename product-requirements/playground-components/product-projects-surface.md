...
The component is exercised at `[data-product-surface=projects]` by `tests/e2e/product-project-pages.acl#project-list-page`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `project-list-page` explicitly targets `[data-product-surface=projects]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-project-pages.acl#project-list-page`, with stable target `[data-product-surface=projects]`.
