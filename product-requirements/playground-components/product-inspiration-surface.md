...
The component is exercised at `[data-product-surface=inspiration]` by `tests/e2e/product-resource-surfaces.acl#inspiration-gallery-to-task`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `inspiration-gallery-to-task` explicitly targets `[data-product-surface=inspiration]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-resource-surfaces.acl#inspiration-gallery-to-task`, with stable target `[data-product-surface=inspiration]`.
