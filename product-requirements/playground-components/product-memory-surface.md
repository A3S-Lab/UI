...
The component is exercised at `[data-product-surface=memory]` by `tests/e2e/product-workspace-capabilities.acl#memory-evolution-workflow`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `memory-evolution-workflow` explicitly targets `[data-product-surface=memory]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#memory-evolution-workflow`, with stable target `[data-product-surface=memory]`.
