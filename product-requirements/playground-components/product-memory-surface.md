# Product Memory Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductMemorySurface` |
| Source | `site/theme/components/playground/ProductMemorySurface.tsx` |
...
| Behavioral scenario | `memory-evolution-workflow` |
| Behavior assertion | `memory-ready` |

## User problem

People using the composed application need to review memories over time, inspect graph relationships, approve proposed evolution, and deliberately reuse context without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Memory Surface owns timeline, graph, and evolution tabs, selected record state, candidate review, graph controls, settings entry, and start-task handoff. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Memory Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-product-surface=memory]` by `tests/e2e/product-workspace-capabilities.acl#memory-evolution-workflow`. The uniquely assigned `memory-ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-product-surface=memory]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Memory Surface must prove candidate acceptance creates traceable provenance, reopening removes generated state consistently, graph filters do not erase records, and compact inspector remains operable; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductMemorySurface` declaration in `site/theme/components/playground/ProductMemorySurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `memory-evolution-workflow` explicitly targets `[data-product-surface=memory]`; its unique `memory-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Memory Surface owns timeline, graph, and evolution tabs, selected record state, candidate review, graph controls, settings entry, and start-task handoff. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Memory Surface must prove candidate acceptance creates traceable provenance, reopening removes generated state consistently, graph filters do not erase records, and compact inspector remains operable; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#memory-evolution-workflow`, assertion `memory-ready`, with stable target `[data-product-surface=memory]`.
