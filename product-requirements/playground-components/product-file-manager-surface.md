# Product File Manager Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductFileManagerSurface` |
| Source | `site/theme/components/playground/ProductFileManagerSurface.tsx` |
...
| Behavioral scenario | `files-mobile-hierarchy` |

## User problem

People using the composed application need to move between task artifacts and a Finder-style workspace while preserving the selected file workflow without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product File Manager Surface owns top-level file tabs, panel semantics, selected workbench handoff, return state, and composition of the public file-manager boundary. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product File Manager Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-product-surface=files]` by `tests/e2e/product-resource-surfaces.acl#files-mobile-hierarchy`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-product-surface=files]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product File Manager Surface must prove switching tabs resets stale workbench selection, compact hierarchy remains navigable, and file operations never imply real filesystem authority in the fixture; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductFileManagerSurface` declaration in `site/theme/components/playground/ProductFileManagerSurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `files-mobile-hierarchy` explicitly targets `[data-product-surface=files]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- The component-specific product boundary remains enforced: Product File Manager Surface owns top-level file tabs, panel semantics, selected workbench handoff, return state, and composition of the public file-manager boundary. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product File Manager Surface must prove switching tabs resets stale workbench selection, compact hierarchy remains navigable, and file operations never imply real filesystem authority in the fixture; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-resource-surfaces.acl#files-mobile-hierarchy`, with stable target `[data-product-surface=files]`.
