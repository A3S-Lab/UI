# Settings Row Product Requirements
...
| Behavior assertion | `shared-row` |

## User problem

People using the composed application need to read a preference name and consequence beside the control that changes it without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Settings Row owns row-level label copy, description, control alignment, responsive stacking, and a reusable semantic grouping boundary. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Settings Row may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-settings__row` by `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`. The uniquely assigned `shared-row` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-settings__row`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Settings Row must prove long descriptions do not detach from controls, compact rows preserve reading order, controls retain their own labels, and no row becomes clickable when only its control is interactive; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `SettingsRow` declaration in `site/theme/components/playground/ProductSettingsPrimitives.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `settings-all-sections` explicitly targets `.product-settings__row`; its unique `shared-row` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Settings Row owns row-level label copy, description, control alignment, responsive stacking, and a reusable semantic grouping boundary. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Settings Row must prove long descriptions do not detach from controls, compact rows preserve reading order, controls retain their own labels, and no row becomes clickable when only its control is interactive; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`, assertion `shared-row`, with stable target `.product-settings__row`.
