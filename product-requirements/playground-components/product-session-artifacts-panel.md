# Product Session Artifacts Panel Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductSessionArtifactsPanel` |
...
| Behavior assertion | `artifacts-panel-boundary` |

## User problem

People using the composed application need to review a produced artifact, inspect highlighted content, and copy it without leaving the session without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Session Artifacts Panel owns artifact list, selected preview, public artifact-card and highlighter composition, copy feedback, file identity, and inspector tab semantics. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Session Artifacts Panel may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-inspector-artifacts` by `tests/e2e/product-session-detail.acl#session-detail-conversation`. The uniquely assigned `artifacts-panel-boundary` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-inspector-artifacts`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Session Artifacts Panel must prove empty artifacts explain absence, copy state is announced, long output remains contained, and selecting another artifact updates both identity and preview; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductSessionArtifactsPanel` declaration in `site/theme/components/playground/ProductSessionArtifactPanels.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `session-detail-conversation` explicitly targets `.product-inspector-artifacts`; its unique `artifacts-panel-boundary` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Session Artifacts Panel owns artifact list, selected preview, public artifact-card and highlighter composition, copy feedback, file identity, and inspector tab semantics. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Session Artifacts Panel must prove empty artifacts explain absence, copy state is announced, long output remains contained, and selecting another artifact updates both identity and preview; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-session-detail.acl#session-detail-conversation`, assertion `artifacts-panel-boundary`, with stable target `.product-inspector-artifacts`.
