# Product Follow Up Queue Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductFollowUpQueue` |
| Source | `site/theme/components/playground/ProductFollowUpQueue.tsx` |
...
| Behavioral scenario | `follow-up-queue-actions` |
| Behavior assertion | `two-items` |

## User problem

People using the composed application need to queue, reorder, edit, pause, remove, and run follow-up instructions while the current task is busy without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Follow Up Queue owns ordered queue presentation, item editing, move controls, pause state, next-run intent, removal, and deterministic status feedback. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Follow Up Queue may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-follow-up-queue` by `tests/e2e/product-action-icon-semantics.acl#follow-up-queue-actions`. The uniquely assigned `two-items` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-follow-up-queue`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Follow Up Queue must prove first and last move controls disable correctly, empty edits cannot save, paused queues cannot look active, and reordering preserves each instruction identity; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductFollowUpQueue` declaration in `site/theme/components/playground/ProductFollowUpQueue.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `follow-up-queue-actions` explicitly targets `.product-follow-up-queue`; its unique `two-items` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Follow Up Queue owns ordered queue presentation, item editing, move controls, pause state, next-run intent, removal, and deterministic status feedback. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Follow Up Queue must prove first and last move controls disable correctly, empty edits cannot save, paused queues cannot look active, and reordering preserves each instruction identity; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-action-icon-semantics.acl#follow-up-queue-actions`, assertion `two-items`, with stable target `.product-follow-up-queue`.
