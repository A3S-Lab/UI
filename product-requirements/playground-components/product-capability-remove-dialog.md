# Product Capability Remove Dialog Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductCapabilityRemoveDialog` |
...
| Behavioral scenario | `capability-overlays-mobile` |
| Behavior assertion | `remove-confirmation-contained` |

## User problem

People using the composed application need to understand the consequences of removing a configured capability and either cancel or confirm explicitly without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Capability Remove Dialog owns the destructive confirmation copy, cancel and confirm actions, modal focus boundary, and return to the originating detail action. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Capability Remove Dialog may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `dialog.product-capability-remove[open]` by `tests/e2e/product-capability-navigation.acl#capability-overlays-mobile`. The uniquely assigned `remove-confirmation-contained` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `dialog.product-capability-remove[open]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Capability Remove Dialog must prove nested dialogs remain correctly layered on phones, cancellation preserves configuration, and the destructive button cannot be mistaken for the safe default; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductCapabilityRemoveDialog` declaration in `site/theme/components/playground/ProductCapabilityManagement.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `capability-overlays-mobile` explicitly targets `dialog.product-capability-remove[open]`; its unique `remove-confirmation-contained` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Capability Remove Dialog owns the destructive confirmation copy, cancel and confirm actions, modal focus boundary, and return to the originating detail action. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Capability Remove Dialog must prove nested dialogs remain correctly layered on phones, cancellation preserves configuration, and the destructive button cannot be mistaken for the safe default; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-capability-navigation.acl#capability-overlays-mobile`, assertion `remove-confirmation-contained`, with stable target `dialog.product-capability-remove[open]`.
