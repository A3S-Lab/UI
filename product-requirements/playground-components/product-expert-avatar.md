# Product Expert Avatar Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductExpertAvatar` |
| Source | `site/theme/components/playground/ProductExpertAvatar.tsx` |
...
| Behavioral scenario | `expert-avatar-composition` |
| Behavior assertion | `assistant-avatar-contract` |

## User problem

People using the composed application need to distinguish configured experts at a glance through stable, composed portraits while relying on nearby text for identity and state without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Expert Avatar owns deterministic palette, hair, facial-detail, and accessory composition across catalog rows, resource pickers, composer chips, and detail views. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Expert Avatar may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-expert-avatar[data-avatar-variant]` by `tests/e2e/product-icon-semantics.acl#expert-avatar-composition`. The uniquely assigned `assistant-avatar-contract` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-expert-avatar[data-avatar-variant]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Expert Avatar must prove all twelve seeded experts retain distinct stable variants across rerenders, decorative portraits remain hidden from assistive technology, and compact rendering never distorts their aspect ratio; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductExpertAvatar` declaration in `site/theme/components/playground/ProductExpertAvatar.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `expert-avatar-composition` explicitly targets `.product-expert-avatar[data-avatar-variant]`; its unique `assistant-avatar-contract` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Expert Avatar owns deterministic palette, hair, facial-detail, and accessory composition across catalog rows, resource pickers, composer chips, and detail views. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Expert Avatar must prove all twelve seeded experts retain distinct stable variants across rerenders, decorative portraits remain hidden from assistive technology, and compact rendering never distorts their aspect ratio; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-icon-semantics.acl#expert-avatar-composition`, assertion `assistant-avatar-contract`, with stable target `.product-expert-avatar[data-avatar-variant]`.
