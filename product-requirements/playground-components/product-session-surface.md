...
The component is exercised at `[data-product-surface=session]` by `tests/e2e/product-task-creation.acl#create-task-desktop`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `create-task-desktop` explicitly targets `[data-product-surface=session]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-task-creation.acl#create-task-desktop`, with stable target `[data-product-surface=session]`.
