...
| Behavior assertion | `product-application-contract` |
...
The component is exercised at `[data-product-application]` by `tests/e2e/product-capability-navigation.acl#global-search-navigation`. The uniquely assigned `product-application-contract` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `global-search-navigation` explicitly targets `[data-product-application]`; its unique `product-application-contract` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- Behavior evidence: `tests/e2e/product-capability-navigation.acl#global-search-navigation`, assertion `product-application-contract`, with stable target `[data-product-application]`.
