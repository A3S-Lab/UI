# Product Settings Dialog Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductSettingsDialog` |
| Source | `site/theme/components/playground/ProductSettingsDialog.tsx` |
...
| Behavior assertion | `settings-ready` |

## User problem

People using the composed application need to navigate every settings domain inside a focused modal and return to the originating product context without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Settings Dialog owns modal lifecycle, section navigation, selected section state, content scrolling, compact layout, Escape handling, close control, and initial-section synchronization. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Settings Dialog may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-settings[open]` by `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`. The uniquely assigned `settings-ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-settings[open]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Settings Dialog must prove all thirteen sections remain reachable, changing sections resets only content scroll, mobile navigation and content do not overlap, and closing leaves no inert or blocked page layer; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductSettingsDialog` declaration in `site/theme/components/playground/ProductSettingsDialog.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `settings-all-sections` explicitly targets `.product-settings[open]`; its unique `settings-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Settings Dialog owns modal lifecycle, section navigation, selected section state, content scrolling, compact layout, Escape handling, close control, and initial-section synchronization. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Settings Dialog must prove all thirteen sections remain reachable, changing sections resets only content scroll, mobile navigation and content do not overlap, and closing leaves no inert or blocked page layer; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`, assertion `settings-ready`, with stable target `.product-settings[open]`.
