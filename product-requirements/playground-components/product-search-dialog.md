...
The component is exercised at `dialog.product-search-dialog[open]` by `tests/e2e/product-capability-navigation.acl#global-search-navigation`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `global-search-navigation` explicitly targets `dialog.product-search-dialog[open]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-capability-navigation.acl#global-search-navigation`, with stable target `dialog.product-search-dialog[open]`.
