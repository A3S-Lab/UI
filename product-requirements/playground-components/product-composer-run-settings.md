...
The component is exercised at `[data-composer-control=run]` by `tests/e2e/product-composer-controls.acl#composer-context-and-run-controls`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `composer-context-and-run-controls` explicitly targets `[data-composer-control=run]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-composer-controls.acl#composer-context-and-run-controls`, with stable target `[data-composer-control=run]`.
