# Product Capability Browse Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductCapabilityBrowse` |
| Source | `site/theme/components/playground/ProductCapabilityBrowse.tsx` |
...
| Behavior assertion | `capability-browse-contract` |

## User problem

People using the composed application need to compare assistants, skills, and connectors using category-appropriate information density and actions without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Capability Browse owns catalog search, filters, grouped results, selection entry points, quick actions, and empty-result explanation for the active capability tab. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Capability Browse may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-product-surface=catalog] [data-directory-layout]` by `tests/e2e/product-capability-navigation.acl#capability-first-navigation-desktop`. The uniquely assigned `capability-browse-contract` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-product-surface=catalog] [data-directory-layout]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Capability Browse must prove tab changes preserve URL truth, filters do not strand focus, and dense skill or connector lists remain scannable without turning every entry into a decorative card; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductCapabilityBrowse` declaration in `site/theme/components/playground/ProductCapabilityBrowse.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `capability-first-navigation-desktop` explicitly targets `[data-product-surface=catalog] [data-directory-layout]`; its unique `capability-browse-contract` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Capability Browse owns catalog search, filters, grouped results, selection entry points, quick actions, and empty-result explanation for the active capability tab. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Capability Browse must prove tab changes preserve URL truth, filters do not strand focus, and dense skill or connector lists remain scannable without turning every entry into a decorative card; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-capability-navigation.acl#capability-first-navigation-desktop`, assertion `capability-browse-contract`, with stable target `[data-product-surface=catalog] [data-directory-layout]`.
