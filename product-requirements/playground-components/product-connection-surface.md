# Product Connection Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductConnectionSurface` |
| Source | `site/theme/components/playground/ProductConnectionSurface.tsx` |
...
| Behavior assertion | `connection-ready` |

## User problem

People using the composed application need to connect a document source, see exactly what access is requested, and recover or disconnect deliberately without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Connection Surface owns inactive, connecting, connected, failed, and disconnect-confirmation fixture states plus provider metadata and host intent events. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Connection Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-connection` by `tests/e2e/product-resource-surfaces.acl#document-connection-lifecycle`. The uniquely assigned `connection-ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-connection`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Connection Surface must prove connection failure retains the provider choice, retry does not fabricate authorization, disconnect requires confirmation, and mobile actions stay adjacent to status; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductConnectionSurface` declaration in `site/theme/components/playground/ProductConnectionSurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `document-connection-lifecycle` explicitly targets `.product-connection`; its unique `connection-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Connection Surface owns inactive, connecting, connected, failed, and disconnect-confirmation fixture states plus provider metadata and host intent events. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Connection Surface must prove connection failure retains the provider choice, retry does not fabricate authorization, disconnect requires confirmation, and mobile actions stay adjacent to status; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-resource-surfaces.acl#document-connection-lifecycle`, assertion `connection-ready`, with stable target `.product-connection`.
