# Product Exit Dialog Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductExitDialog` |
| Source | `site/theme/components/playground/ProductOverlayMenus.tsx` |
...
| Behavioral scenario | `account-menu-and-exit` |
| Behavior assertion | `exit-confirmation` |

## User problem

People using the composed application need to understand what stopping the local fixture means and deliberately cancel or confirm the exit request without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Exit Dialog owns the modal warning, retained-draft explanation, safe and destructive actions, backdrop and Escape cancellation, application exit event, and focus return. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Exit Dialog may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `dialog.product-exit-dialog[open]` by `tests/e2e/product-global-and-settings-contracts.acl#account-menu-and-exit`. The uniquely assigned `exit-confirmation` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `dialog.product-exit-dialog[open]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Exit Dialog must prove cancel returns focus to the account trigger, confirmation cannot be accidental, compact text remains readable, and closing never leaves an invisible modal blocking the page; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductExitDialog` declaration in `site/theme/components/playground/ProductOverlayMenus.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `account-menu-and-exit` explicitly targets `dialog.product-exit-dialog[open]`; its unique `exit-confirmation` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Exit Dialog owns the modal warning, retained-draft explanation, safe and destructive actions, backdrop and Escape cancellation, application exit event, and focus return. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Exit Dialog must prove cancel returns focus to the account trigger, confirmation cannot be accidental, compact text remains readable, and closing never leaves an invisible modal blocking the page; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#account-menu-and-exit`, assertion `exit-confirmation`, with stable target `dialog.product-exit-dialog[open]`.
