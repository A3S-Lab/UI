...
The component is exercised at `dialog.product-exit-dialog[open]` by `tests/e2e/product-global-and-settings-contracts.acl#account-menu-and-exit`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `account-menu-and-exit` explicitly targets `dialog.product-exit-dialog[open]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#account-menu-and-exit`, with stable target `dialog.product-exit-dialog[open]`.
