...
The component is exercised at `.product-project-presence` by `tests/e2e/product-project-workflow.acl#project-workspace-and-session`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `project-workspace-and-session` explicitly targets `.product-project-presence` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-project-workflow.acl#project-workspace-and-session`, with stable target `.product-project-presence`.
