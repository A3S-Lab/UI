# Product Composer Mode Control Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductComposerModeControl` |
...
| Behavior assertion | `automatic-mode-selected` |

## User problem

People using the composed application need to choose the task mode and permission posture without translating opaque implementation flags without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Composer Mode Control owns mode options, permission guidance, selected state, keyboard navigation, disclosure close behavior, and the controlled mode callback. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Composer Mode Control may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-composer-control=mode]` by `tests/e2e/product-composer-controls.acl#composer-context-and-run-controls`. The uniquely assigned `automatic-mode-selected` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-composer-control=mode]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Composer Mode Control must prove read-only and editing permissions cannot be visually conflated, disabled modes remain skipped, and changing modes never discards the current draft; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductComposerModeControl` declaration in `site/theme/components/playground/ProductComposerRunSettings.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `composer-context-and-run-controls` explicitly targets `[data-composer-control=mode]`; its unique `automatic-mode-selected` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Composer Mode Control owns mode options, permission guidance, selected state, keyboard navigation, disclosure close behavior, and the controlled mode callback. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Composer Mode Control must prove read-only and editing permissions cannot be visually conflated, disabled modes remain skipped, and changing modes never discards the current draft; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-composer-controls.acl#composer-context-and-run-controls`, assertion `automatic-mode-selected`, with stable target `[data-composer-control=mode]`.
