...
The component is exercised at `[data-product-surface=automation-builder]` by `tests/e2e/product-automation-and-preview.acl#automation-builder-workflow`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `automation-builder-workflow` explicitly targets `[data-product-surface=automation-builder]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-automation-and-preview.acl#automation-builder-workflow`, with stable target `[data-product-surface=automation-builder]`.
