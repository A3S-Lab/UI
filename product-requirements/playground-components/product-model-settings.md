...
The component is exercised at `.product-model-toolbar` by `tests/e2e/product-model-settings.acl#model-provider-workflow`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `model-provider-workflow` explicitly targets `.product-model-toolbar` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-model-settings.acl#model-provider-workflow`, with stable target `.product-model-toolbar`.
