# Empty Workspace Panel Product Requirements

| Field | Contract |
| --- | --- |
| Export | `EmptyWorkspacePanel` |
...
| Boundary scenario | `playground-legacy-empty-workspace-panel` |
| Boundary assertion | `legacy-workspace-absent` |

## User problem

Maintainers need to know that Empty Workspace Panel belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Empty Workspace Panel may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is the watermark for the retired dock manager and is not a valid empty state for any current route.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-empty-workspace-panel` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Empty Workspace Panel must prove the current Playground never substitutes a generic editor watermark for route-specific empty, recovery, or first-use states; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `EmptyWorkspacePanel` declaration in `site/theme/components/playground/WorkspacePanels.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-empty-workspace-panel` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Empty Workspace Panel may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is the watermark for the retired dock manager and is not a valid empty state for any current route.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Empty Workspace Panel must prove the current Playground never substitutes a generic editor watermark for route-specific empty, recovery, or first-use states; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-empty-workspace-panel`, plus the generator's active-import rejection.
