...
The component is exercised at `[data-product-surface=marketplace]` by `tests/e2e/product-workspace-capabilities.acl#extension-install-lifecycle`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `extension-install-lifecycle` explicitly targets `[data-product-surface=marketplace]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#extension-install-lifecycle`, with stable target `[data-product-surface=marketplace]`.
