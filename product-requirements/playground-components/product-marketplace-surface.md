# Product Marketplace Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductMarketplaceSurface` |
| Source | `site/theme/components/playground/ProductMarketplaceSurface.tsx` |
...
| Behavior assertion | `marketplace-ready` |

## User problem

People using the composed application need to discover extensions and manage their lifecycle while separating directory browsing, source trust, detail review, and isolated execution without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Marketplace Surface owns marketplace tabs, search and sorting, selected extension, install state, source management composition, detail dialog mode, and host entry. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Marketplace Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
Component-specific adversarial requirement: Acceptance for Product Marketplace Surface must prove installed state survives deterministic reload, source trust remains visible, compact detail does not cover global recovery, and sorting never changes semantic selection; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductMarketplaceSurface` declaration in `site/theme/components/playground/ProductMarketplaceSurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `extension-install-lifecycle` explicitly targets `[data-product-surface=marketplace]`; its unique `marketplace-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Marketplace Surface owns marketplace tabs, search and sorting, selected extension, install state, source management composition, detail dialog mode, and host entry. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#extension-install-lifecycle`, assertion `marketplace-ready`, with stable target `[data-product-surface=marketplace]`.
