# Scene State Product Requirements

| Field | Contract |
| --- | --- |
| Export | `SceneState` |
| Source | `site/theme/components/playground/SceneState.tsx` |
...
| Boundary scenario | `playground-legacy-scene-state` |

## User problem

Maintainers need to know that Scene State belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Scene State may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It describes loading and error states only for the retired scene canvas, not for current task, session, or resource surfaces.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-scene-state` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Scene State must prove current routes express their own empty and failure states and never mount the old scene-state panel; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `SceneState` declaration in `site/theme/components/playground/SceneState.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-scene-state` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Scene State may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It describes loading and error states only for the retired scene canvas, not for current task, session, or resource surfaces.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Scene State must prove current routes express their own empty and failure states and never mount the old scene-state panel; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-scene-state`, plus the generator's active-import rejection.
