# Playground Icon Product Requirements

| Field | Contract |
| --- | --- |
| Export | `PlaygroundIcon` |
| Source | `site/theme/components/playground/PlaygroundIcon.tsx` |
...
| Boundary scenario | `playground-legacy-playground-icon` |

## User problem

Maintainers need to know that Playground Icon belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Playground Icon may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. Its icon vocabulary serves only the retired workspace scene; current product fixtures use Product Playground Icon with audited action semantics.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-playground-icon` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Playground Icon must prove no current route imports the retired icon renderer and its glyphs cannot reappear through an indirect workspace panel import; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `PlaygroundIcon` declaration in `site/theme/components/playground/PlaygroundIcon.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-playground-icon` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Playground Icon may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. Its icon vocabulary serves only the retired workspace scene; current product fixtures use Product Playground Icon with audited action semantics.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Playground Icon must prove no current route imports the retired icon renderer and its glyphs cannot reappear through an indirect workspace panel import; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-playground-icon`, plus the generator's active-import rejection.
