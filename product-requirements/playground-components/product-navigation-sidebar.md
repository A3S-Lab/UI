...
The component is exercised at `.product-sidebar` by `tests/e2e/product-workspace-capabilities.acl#sidebar-control-semantics`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `sidebar-control-semantics` explicitly targets `.product-sidebar` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#sidebar-control-semantics`, with stable target `.product-sidebar`.
