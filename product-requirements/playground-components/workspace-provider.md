# Workspace Provider Product Requirements

| Field | Contract |
| --- | --- |
| Export | `WorkspaceProvider` |
| Source | `site/theme/components/playground/WorkspaceContext.tsx` |
...
Maintainers need to know that Workspace Provider belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Workspace Provider may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It provides scene and workspace state only to the retired editor-style panel tree and has no current product consumer.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-workspace-provider` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Workspace Provider must prove the current application import graph remains independent of the provider even though a provider produces no detectable DOM node; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `WorkspaceProvider` declaration in `site/theme/components/playground/WorkspaceContext.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-workspace-provider` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Workspace Provider may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It provides scene and workspace state only to the retired editor-style panel tree and has no current product consumer.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Workspace Provider must prove the current application import graph remains independent of the provider even though a provider produces no detectable DOM node; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-workspace-provider`, plus the generator's active-import rejection.
