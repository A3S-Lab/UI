...
The component is exercised at `.product-file-artifacts` by `tests/e2e/product-resource-surfaces.acl#files-mobile-hierarchy`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `files-mobile-hierarchy` explicitly targets `.product-file-artifacts` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-resource-surfaces.acl#files-mobile-hierarchy`, with stable target `.product-file-artifacts`.
