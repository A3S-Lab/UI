# Product File Artifacts Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductFileArtifactsSurface` |
| Source | `site/theme/components/playground/ProductFileArtifactsSurface.tsx` |
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product File Artifacts Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-file-artifacts` by `tests/e2e/product-resource-surfaces.acl#files-mobile-hierarchy`. The uniquely assigned `artifacts-ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-file-artifacts`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product File Artifacts Surface must prove zero results keep filters editable, long names do not collapse metadata, keyboard table actions remain reachable, and the default view is not confused with the workspace filesystem; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductFileArtifactsSurface` declaration in `site/theme/components/playground/ProductFileArtifactsSurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `files-mobile-hierarchy` explicitly targets `.product-file-artifacts`; its unique `artifacts-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product File Artifacts Surface owns artifact filters, search, grouped table disclosure, result count, open-workspace intent, and start-task intent over deterministic artifacts. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product File Artifacts Surface must prove zero results keep filters editable, long names do not collapse metadata, keyboard table actions remain reachable, and the default view is not confused with the workspace filesystem; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-resource-surfaces.acl#files-mobile-hierarchy`, assertion `artifacts-ready`, with stable target `.product-file-artifacts`.
