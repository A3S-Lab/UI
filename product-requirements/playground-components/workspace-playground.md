# Workspace Playground Product Requirements

| Field | Contract |
| --- | --- |
| Export | `WorkspacePlayground` |
| Source | `site/theme/components/playground/WorkspacePlayground.tsx` |
...
| Boundary scenario | `playground-legacy-workspace-playground` |
| Boundary assertion | `legacy-workspace-absent` |

## User problem

Maintainers need to know that Workspace Playground belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Workspace Playground may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is the retired root that assembled movable editor panels and must not be used as the current Playground route shell.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-workspace-playground` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Workspace Playground must prove all current routes render Product Application directly and the retired workspace root stays absent at desktop and phone widths; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `WorkspacePlayground` declaration in `site/theme/components/playground/WorkspacePlayground.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-workspace-playground` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Workspace Playground may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is the retired root that assembled movable editor panels and must not be used as the current Playground route shell.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Workspace Playground must prove all current routes render Product Application directly and the retired workspace root stays absent at desktop and phone widths; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-workspace-playground`, plus the generator's active-import rejection.
