...
The component is exercised at `[data-product-surface=assistant]` by `tests/e2e/product-global-and-settings-contracts.acl#assistant-surface-controls`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `assistant-surface-controls` explicitly targets `[data-product-surface=assistant]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#assistant-surface-controls`, with stable target `[data-product-surface=assistant]`.
