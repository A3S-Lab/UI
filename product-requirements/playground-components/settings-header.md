...
| Behavior assertion | `system-header` |
...
- The real interaction path `settings-all-sections` explicitly targets `.product-settings__content-header`; its unique `system-header` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`, assertion `system-header`, with stable target `.product-settings__content-header`.
