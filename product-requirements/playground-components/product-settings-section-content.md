...
The component is exercised at `.product-settings[open] main` by `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `settings-all-sections` explicitly targets `.product-settings[open] main` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`, with stable target `.product-settings[open] main`.
