# Product Settings Section Content Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductSettingsSectionContent` |
| Source | `site/theme/components/playground/ProductSettingsSections.tsx` |
...
| Behavior assertion | `settings-section-content` |

## User problem

People using the composed application need to render exactly the settings component selected in navigation without leaking stale controls from another section without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Settings Section Content owns typed settings-section dispatch, locale propagation, and one active content subtree within the owning dialog. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Settings Section Content may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-settings[open] main` by `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`. The uniquely assigned `settings-section-content` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-settings[open] main`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Settings Section Content must prove every section identifier resolves, switching cannot leave duplicate headings or focusable stale controls, and unknown values fail during development instead of showing the wrong page; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductSettingsSectionContent` declaration in `site/theme/components/playground/ProductSettingsSections.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `settings-all-sections` explicitly targets `.product-settings[open] main`; its unique `settings-section-content` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Settings Section Content owns typed settings-section dispatch, locale propagation, and one active content subtree within the owning dialog. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Settings Section Content must prove every section identifier resolves, switching cannot leave duplicate headings or focusable stale controls, and unknown values fail during development instead of showing the wrong page; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`, assertion `settings-section-content`, with stable target `.product-settings[open] main`.
