# Task Panel Product Requirements
...
| Boundary scenario | `playground-legacy-task-panel` |

## User problem

Maintainers need to know that Task Panel belongs only to the retired editor-style workspace tree so it cannot silently reappear in the current task-first Playground. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Task Panel may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is a dock panel from the retired workspace topology; current task input and evidence are owned by task and session surfaces.
...
The export has no supported visible target. `tests/e2e/playground-component-boundaries.acl#playground-legacy-task-panel` proves the current application renders without `.a3s-workspace-playground` at desktop and phone widths, while the generator rejects any active import of this legacy export.
...
Component-specific adversarial requirement: The boundary test for Task Panel must prove no current route duplicates the composer or task detail inside an editor-style dock panel; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- The export inventory contains exactly one `TaskPanel` declaration in `site/theme/components/playground/WorkspacePanels.tsx` and exactly one matching PRD and coverage record.
...
- The generated boundary scenario `playground-legacy-task-panel` proves the current Product Application does not mount the retired workspace root.
...
- The component-specific product boundary remains enforced: Task Panel may remain temporarily as source-compatible legacy code for its existing private consumers, while the current application owns no route, navigation item, product promise, or visible state for it. It is a dock panel from the retired workspace topology; current task input and evidence are owned by task and session surfaces.
- The adversarial release condition passes without weakening selectors or assertions: The boundary test for Task Panel must prove no current route duplicates the composer or task detail inside an editor-style dock panel; a passing result cannot rely on hiding the obsolete workspace with CSS or renaming its visible chrome.
...
- Boundary evidence: `tests/e2e/playground-component-boundaries.acl#playground-legacy-task-panel`, plus the generator's active-import rejection.
