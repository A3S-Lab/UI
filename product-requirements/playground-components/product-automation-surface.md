# Product Automation Surface Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductAutomationSurface` |
| Source | `site/theme/components/playground/ProductAutomationSurface.tsx` |
...
| Behavioral scenario | `paused-manual-run` |
| Behavior assertion | `automation-ready` |

## User problem

People using the composed application need to scan automations, understand schedule and health, and enter builder or history work without confusing definitions with runs without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Automation Surface owns automation tabs, deterministic template and list composition, selected definition state, pause intent, manual-run intent, and builder disclosure. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Automation Surface may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `[data-product-surface=automation]` by `tests/e2e/product-automation-and-preview.acl#paused-manual-run`. The uniquely assigned `automation-ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `[data-product-surface=automation]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Automation Surface must prove paused items cannot imply an active schedule, manual runs expose pending feedback, and master-detail content remains reachable on phones; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductAutomationSurface` declaration in `site/theme/components/playground/ProductAutomationSurface.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `paused-manual-run` explicitly targets `[data-product-surface=automation]`; its unique `automation-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Automation Surface owns automation tabs, deterministic template and list composition, selected definition state, pause intent, manual-run intent, and builder disclosure. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Automation Surface must prove paused items cannot imply an active schedule, manual runs expose pending feedback, and master-detail content remains reachable on phones; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-automation-and-preview.acl#paused-manual-run`, assertion `automation-ready`, with stable target `[data-product-surface=automation]`.
