...
The component is exercised at `[data-capability-id] img, [data-capability-id] svg[data-icon]` by `tests/e2e/product-icon-semantics.acl#skill-card-icons`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `skill-card-icons` explicitly targets `[data-capability-id] img, [data-capability-id] svg[data-icon]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-icon-semantics.acl#skill-card-icons`, with stable target `[data-capability-id] img, [data-capability-id] svg[data-icon]`.
