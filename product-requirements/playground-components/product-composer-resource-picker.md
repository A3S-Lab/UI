...
The component is exercised at `[data-composer-resource-picker]` by `tests/e2e/product-action-icon-semantics.acl#composer-capability-actions`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `composer-capability-actions` explicitly targets `[data-composer-resource-picker]` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-action-icon-semantics.acl#composer-capability-actions`, with stable target `[data-composer-resource-picker]`.
