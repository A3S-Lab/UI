...
| Behavior assertion | `start-surface-boundary` |
...
- The real interaction path `create-task-desktop` explicitly targets `[data-product-surface=start]`; its unique `start-surface-boundary` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- Behavior evidence: `tests/e2e/product-task-creation.acl#create-task-desktop`, assertion `start-surface-boundary`, with stable target `[data-product-surface=start]`.
