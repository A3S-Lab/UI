...
The component is exercised at `.product-project-assets-workspace` by `tests/e2e/product-project-asset-preview.acl#project-asset-preview-desktop`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `project-asset-preview-desktop` explicitly targets `.product-project-assets-workspace` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-project-asset-preview.acl#project-asset-preview-desktop`, with stable target `.product-project-assets-workspace`.
