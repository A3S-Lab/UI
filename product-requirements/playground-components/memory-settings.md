# Memory Settings Product Requirements
...
| Behavior assertion | `memory-section` |

## User problem

People using the composed application need to set how future memories are created and scoped while keeping record review and deletion in the memory workspace without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Memory Settings owns memory enablement, default scope, policy explanation, shared rows and switches, and setting-change announcements. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Memory Settings may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-settings__boundary` by `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`. The uniquely assigned `memory-section` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
Component-specific adversarial requirement: Acceptance for Memory Settings must prove disabling memory does not pretend to delete records, scope changes remain explicit, and policy copy prevents settings from becoming a parallel memory manager; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `MemorySettings` declaration in `site/theme/components/playground/ProductSettingsIntelligenceSections.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `settings-all-sections` explicitly targets `.product-settings__boundary`; its unique `memory-section` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- Behavior evidence: `tests/e2e/product-global-and-settings-contracts.acl#settings-all-sections`, assertion `memory-section`, with stable target `.product-settings__boundary`.
