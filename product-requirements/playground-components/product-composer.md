# Product Composer Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductComposer` |
| Source | `site/theme/components/playground/ProductComposer.tsx` |
...
| Behavior assertion | `composer-ready` |

## User problem

People using the composed application need to express a task, attach bounded context, choose execution policy, and submit through one predictable input surface without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Composer owns the rich-text task entry boundary, attachment and run-control orchestration, drag-and-drop intake intent, submission eligibility, and host callbacks. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Composer may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `.product-composer` by `tests/e2e/product-composer-controls.acl#composer-context-and-run-controls`. The uniquely assigned `composer-ready` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `.product-composer`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Composer must prove focus uses a calm border instead of a double ring, empty or composing text cannot submit, dropped files remain explicit, and overlays never cover the active input; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductComposer` declaration in `site/theme/components/playground/ProductComposer.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `composer-context-and-run-controls` explicitly targets `.product-composer`; its unique `composer-ready` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Composer owns the rich-text task entry boundary, attachment and run-control orchestration, drag-and-drop intake intent, submission eligibility, and host callbacks. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Composer must prove focus uses a calm border instead of a double ring, empty or composing text cannot submit, dropped files remain explicit, and overlays never cover the active input; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-composer-controls.acl#composer-context-and-run-controls`, assertion `composer-ready`, with stable target `.product-composer`.
