...
The component is exercised at `.product-file-workbench` by `tests/e2e/product-workspace-capabilities.acl#workspace-file-intake-and-code-roundtrip`. That scenario owns the state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.
...
- The real interaction path `workspace-file-intake-and-code-roundtrip` explicitly targets `.product-file-workbench` and proves a state-changing keyboard or pointer action in the same component-owned workflow.
...
- Behavior evidence: `tests/e2e/product-workspace-capabilities.acl#workspace-file-intake-and-code-roundtrip`, with stable target `.product-file-workbench`.
