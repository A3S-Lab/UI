# Product Capability Detail Dialog Product Requirements

| Field | Contract |
| --- | --- |
| Export | `ProductCapabilityDetailDialog` |
| Source | `site/theme/components/playground/ProductCapabilityManagement.tsx` |
...
| Behavioral scenario | `capability-first-navigation-desktop` |
| Behavior assertion | `assistant-detail` |

## User problem

People using the composed application need to inspect capability purpose, state, permissions, and next valid action before changing configuration without losing the task, resource, or decision context that made the action meaningful. This internal component is justified only as a reusable, deterministic composition boundary. Its requirements must describe the decision it supports rather than restating its export name or visual shape.

## Product boundary

Product Capability Detail Dialog owns modal detail hierarchy, status and provenance presentation, primary and destructive entry points, Escape handling, and focus return. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
...
Required states are ready, focused, selected or expanded where applicable, compact, localized, dark, empty, unavailable, invalid, pending, successful, and failed. Only states owned by Product Capability Detail Dialog may change its local DOM; host-owned progress and outcomes remain explicit fixture inputs.
...
The component is exercised at `dialog.product-capability-detail-dialog[open]` by `tests/e2e/product-capability-navigation.acl#capability-first-navigation-desktop`. The uniquely assigned `assistant-detail` action directly verifies this component inside that state-changing path; the route scenario independently owns direct-load, responsive, accessibility, console, page-error, and screenshot evidence.

Pointer and keyboard paths must reach the same decision. Focus enters through a named native control, movement follows the widget's documented orientation, Escape cancels a transient layer, and focus returns to the exact trigger. The component target is `dialog.product-capability-detail-dialog[open]`; incidental descendants are not a supported automation API.
...
Component-specific adversarial requirement: Acceptance for Product Capability Detail Dialog must prove short viewports keep the title and footer reachable, destructive actions never bypass confirmation, and closing returns to the exact catalog entry; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- The export inventory contains exactly one `ProductCapabilityDetailDialog` declaration in `site/theme/components/playground/ProductCapabilityManagement.tsx` and exactly one matching PRD and coverage record.
...
- The real interaction path `capability-first-navigation-desktop` explicitly targets `dialog.product-capability-detail-dialog[open]`; its unique `assistant-detail` assertion verifies this component while the same workflow proves a state-changing keyboard or pointer action.
...
- The component-specific product boundary remains enforced: Product Capability Detail Dialog owns modal detail hierarchy, status and provenance presentation, primary and destructive entry points, Escape handling, and focus return. It does not own product routing services, remote APIs, credentials, authorization, persistence, scheduling, or irreversible side effects.
- The adversarial release condition passes without weakening selectors or assertions: Acceptance for Product Capability Detail Dialog must prove short viewports keep the title and footer reachable, destructive actions never bypass confirmation, and closing returns to the exact catalog entry; malformed, stale, denied, empty, and excessively long fixture data must remain contained without fabricating host success.
...
- Behavior evidence: `tests/e2e/product-capability-navigation.acl#capability-first-navigation-desktop`, assertion `assistant-detail`, with stable target `dialog.product-capability-detail-dialog[open]`.
