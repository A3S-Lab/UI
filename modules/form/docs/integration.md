...
Import `@a3s-lab/ui/form/a3s-ui.css` once in A3S products and standalone surfaces. It combines the published A3S UI 0.4.1 stylesheet with the Form layout layer. Fields use the documented A3S UI `field`, `input`, `textarea`, `select`, `fieldset`, `btn`, `tabs`, `accordion`, `table`, and `progress` contracts.
