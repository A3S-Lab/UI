...
The bundled Router form in the Playground follows this migration: `routes` remains the same metadata-free workflow configuration array, but each condition is edited through a repeatable row with row-level validation and a host-owned branch data source.
