...
- Host-neutral workflow-node configuration, digest-pinned validation, controlled adapter parity, scoped CSS, and package budgets are implemented and covered by the reference workflow host.
...
- Publish a host-neutral workflow-node configuration contract for embedded settings panels, including node identity, a configuration-mode `FormRef`, controlled values, locale/read-only context, and digest-pinned validation.
...
- The tested reference host can render and commit a workflow-node configuration without importing A3S Cloud, A3S Workflow runtime services, or global UI resets.
...
- The Router workflow-node sample uses a metadata-free editable data grid for `routes`, per-row validation, and host-owned branch options, proving the embedding path with ordinary node configuration values.
- A committed A3S Test flow covers empty state, dialog add/edit/cancel, TSV append, fill-down, row-bound options, validation focus, reorder, multiple selection, confirmed deletion, semantic output, and desktop/390 px Chrome layouts for the Router grid.
