...
const previewDevicePresets = [
  {
    ...
    frame: "phone",
    id: "phone-compact",
    label: { en: "Compact phone", zh: "紧凑手机" },
  },
  {
    dimensions: "412 × 915",
    frame: "phone",
    id: "phone-large",
    label: { en: "Large phone", zh: "大屏手机" },
  },
  {
    dimensions: "820 × 1180",
    frame: "tablet",
    id: "tablet",
    label: { en: "Tablet", zh: "平板" },
  },
  {
    dimensions: "1366 × 768",
    frame: "desktop",
    id: "laptop",
    label: { en: "Laptop", zh: "笔记本" },
  },
  {
    dimensions: "1440 × 900",
    frame: "desktop",
    id: "desktop",
    label: { en: "Desktop", zh: "桌面" },
  },
] as const;
...
export function ProductSessionPreviewPanel({
  ...
  const activePreset =
    previewDevicePresets.find((preset) => preset.id === device) ??
    previewDevicePresets[0];
    ...
    >
      {expanded ? (
        ...
        <>
          <header className="product-inspector-preview__toolbar">
            <label>
              <ProductPlaygroundIcon
                name={
                  activePreset.frame === "desktop"
                    ? "workspace"
                    : activePreset.frame === "tablet"
                      ? "document"
                      : "video"
                }
              />
              <span className="sr-only">
                {zh ? "预览设备尺寸" : "Preview device size"}
              </span>
              <select
                aria-label={zh ? "预览设备尺寸" : "Preview device size"}
                onChange={(event) =>
                  setDevice(event.currentTarget.value as PreviewDevicePresetId)
                }
                value={device}
              >
                {previewDevicePresets.map((preset) => (
                  <option key={preset.id} value={preset.id}>
                    {preset.label[locale]} · {preset.dimensions}
                  </option>
                ))}
              </select>
            </label>
          ...
          <DeviceShell
            device={activePreset.frame}
            key={`${device}-${previewRevision}`}
            locale={locale}
            preset={device}
          />
          <footer>
            ...
            <small>
              {activePreset.label[locale]} · {activePreset.dimensions}
            </small>
          </footer>
        </>
      )}
...
function DeviceShell({
  ...
  return (
    <div
      className="product-device-stage"
      data-device={device}
      data-preset={preset}
    >
      <div className="product-device-shell">
        ...
        <div className="product-device-shell__screen">
          <div className="product-device-shell__sensor">
            <i />
            <span />
          </div>
          <header>
          ...
          <main>
            <div>
              ...
              <strong>A3S</strong>
              <small>{zh ? "本地预览" : "Local preview"}</small>
            </div>
            <section>
              ...
              <p>
                {zh
                  ? "返回路由与键盘焦点均已保留。"
                  : "Return routing and keyboard focus are preserved."}
              </p>
              <ul>
                <li>
                  <ProductPlaygroundIcon name="check" />
                  {zh ? "路由检查" : "Route check"}
                  <span>{zh ? "通过" : "Passed"}</span>
                </li>
                <li>
                  <ProductPlaygroundIcon name="check" />
                  {zh ? "焦点恢复" : "Focus recovery"}
                  <span>{zh ? "通过" : "Passed"}</span>
                </li>
              </ul>
              <button type="button">
                {zh ? "查看验证证据" : "Review evidence"}
              </button>
            </section>
          </main>
          <footer>
        ...
        <div className="product-device-shell__stand">
          <i />
        </div>
      </div>
    </div>
