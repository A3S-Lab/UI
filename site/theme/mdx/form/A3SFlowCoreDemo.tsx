...
const visibleManifests = a3sFlowDagNodeManifestCatalog.filter(
...
export function A3SFlowDagDemo() {
  ...
  const [dagNode, setDagNode] = useState(() =>
    createA3SFlowDagNode(
      "docs-flow-step",
      requireA3SFlowDagNodeManifest(initialType),
    ),
  );
  ...
  const [status, setStatus] = useState(
    isEnglish
      ? "Changes update the DAG node preview."
      : "修改配置后，DAG 节点预览会同步更新。",
  );
      ...
      ...new Map(
        filteredManifests.map((candidate) => [
          candidate.category,
          {
            ...
            items: filteredManifests.filter(
  ...
  const selectNode = (nextType: string) => {
    ...
    setNodeStatus("idle");
    setStatus(
      isEnglish ? `${nextLabel} selected.` : `已选择「${nextLabel}」。`,
    );
  };
  ...
  const runNode = async () => {
    ...
    setStatus(
      isEnglish
        ? `${selectedLabel} completed successfully.`
        : `「${selectedLabel}」运行成功。`,
    );
  };
        ...
        <div className="a3s-doc-workflow-library">
          ...
          <div className="a3s-doc-workflow-library__tabs" role="tablist">
            ...
            <span>
              ...
              {A3S_FLOW_DAG_NODE_MANIFEST_PROVENANCE.runtimeCommands}
            </span>
          </div>
                ...
                {category.items.map((candidate) => {
                  ...
                  return (
                    <button
                      ...
                      data-selected={
                        candidate.type === dagNode.data.type || undefined
                      }
