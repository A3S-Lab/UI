...
Accordion不公开自定义方法或事件。使用原生`details.open`布尔值以编程方式打开或关闭项目。
...
export function Example() {
  ...
  return (
    <Accordion ref={control.ref} aria-label="Accordion">组件内容</Accordion>
  );
}
```
...
<template>
  <Accordion ref="componentRef" aria-label="Accordion">组件内容</Accordion>
</template>
```
