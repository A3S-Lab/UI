...
A3S UI未实现Vaul的拖动关闭呼吸、捕捉点或拖动手柄。滑块通过外部单击、Escape或显式关闭控件关闭。
</Callout>
...
export function Example() {
  ...
  return (
    <Drawer ref={control.ref} aria-label="Drawer"><div><header><h2>组件摘要</h2></header><section>组件内容</section><footer><button type="button">继续</button></footer></div></Drawer>
  );
}
```

`useDrawer` 订阅清单中声明的 DOM 事件，并且只调用清单中公开的方法。适配器不创建另一套框架专属状态。
...
<template>
  <Drawer ref="componentRef" aria-label="Drawer"><div><header><h2>组件摘要</h2></header><section>组件内容</section><footer><button type="button">继续</button></footer></div></Drawer>
</template>
```
