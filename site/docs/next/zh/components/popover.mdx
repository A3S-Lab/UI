...
<dl>
  ...
  <dd>
    ...
    <dl>
      ...
      <dd>Popover 内容。支持 <code>data-side="top|right|bottom|left|inline-start|inline-end"</code> 与 <code>data-align="start|center|end"</code>。默认使用 8px 侧边偏移和 8px 视口碰撞留白；可通过 <code>data-side-offset</code> 与 <code>data-collision-padding</code> 覆盖，或设置 <code>data-collision="none"</code> 保留静态 CSS 定位。定位期间运行时会发布 <code>--available-height</code>、<code>data-resolved-side</code>、<code>data-resolved-align</code> 与 <code>data-a3s-positioned="true"</code>。</dd>
    </dl>
  </dd>
</dl>
