...
Alert Dialog 保留原生 `HTMLDialogElement` 契约。React 的 `useAlertDialog` 与 Vue 的同名 composable 可调用 `showModal()` 和 `close(returnValue)`；`cancel` 与 `close` 仍是原生 DOM 事件。
...
export function Example() {
  ...
  return (
    <AlertDialog ref={control.ref} aria-label="Alert Dialog"><div><header><h2>组件摘要</h2></header><section>组件内容</section><footer><button type="button">继续</button></footer></div></AlertDialog>
  );
}
```
...
<template>
  <AlertDialog ref="componentRef" aria-label="Alert Dialog"><div><header><h2>组件摘要</h2></header><section>组件内容</section><footer><button type="button">继续</button></footer></div></AlertDialog>
</template>
```
