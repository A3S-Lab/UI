...
```tsx
import { Chart, useChart } from "@a3s-lab/ui/react";

export function Example() {
  const control = useChart();
  return (
    <Chart ref={control.ref}>组件内容</Chart>
  );
}
```

`useChart` 订阅清单中声明的 DOM 事件，并且只调用清单中公开的方法。适配器不创建另一套框架专属状态。

## Vue

```vue
<script setup>
import { Chart, useChart } from "@a3s-lab/ui/vue";
...
<template>
  <Chart ref="componentRef">组件内容</Chart>
</template>
```
