...
| `@a3s-lab/ui/compat` | Optional pre-1.0 Basecoat compatibility aliases |
...
| `@a3s-lab/ui/basecoat` | Shared lifecycle and registry |
...
The public runtime namespace remains `window.basecoat` for compatibility. It initializes registered components on page load and observes newly inserted DOM.

```js
window.basecoat.initAll();
window.basecoat.initAll({ force: true });
window.basecoat.refresh(document.querySelector(".split-pane"));
```
