...
export function Example() {
  ...
  return (
    <Chart ref={control.ref}>Component content</Chart>
  );
}
```

`useChart` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Chart ref="componentRef">Component content</Chart>
</template>
```
