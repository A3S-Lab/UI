...
export function Example() {
  ...
  return (
    <Command ref={control.ref}>Component content</Command>
  );
}
```

`useCommand` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Command ref="componentRef">Component content</Command>
</template>
```
