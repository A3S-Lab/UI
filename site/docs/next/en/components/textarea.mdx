...
Textarea captures longer, potentially multiline content and should keep length, formatting, or validation requirements visible.
...
<Preview class="w-full max-w-xs">
<textarea class="textarea" aria-label="Message" placeholder="Type your message here" disabled></textarea>
</Preview>
...
export function Example() {
  ...
  return (
    <Textarea ref={control.ref} aria-label="Name" defaultValue="Component content" />
  );
}
```

`useTextarea` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Textarea ref="componentRef" aria-label="Name" value="Component content" />
</template>
```
