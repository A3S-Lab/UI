...
Slider lets users adjust a value quickly across a continuous or stepped range when the result is immediately perceptible and exact typing is unnecessary.
...
export function Example() {
  ...
  return (
    <Slider ref={control.ref} aria-label="Name" />
  );
}
```

`useSlider` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Slider ref="componentRef" aria-label="Name" />
</template>
```
