...
export function Example() {
  ...
  return (
    <Checkbox ref={control.ref} aria-label="Name" />
  );
}
```

`useCheckbox` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Checkbox ref="componentRef" aria-label="Name" />
</template>
```
