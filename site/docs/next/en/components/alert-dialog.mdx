...
Alert Dialog requires an explicit choice before a high-risk or irreversible action. Use it only when continuing without a response would be unsafe.

<Preview class="w-full max-w-lg">
<div class="grid w-full justify-items-start gap-4">
  <button type="button" onclick="const dialog = document.getElementById('demo-alert-dialog'); if (dialog.open) dialog.close(); dialog.style.cssText = ''; dialog.firstElementChild.style.cssText = ''; dialog.showModal()" class="btn" data-variant="outline">Open confirmation dialog</button>
  ...
  <div style="position:relative;inset:auto;translate:none;scale:1;transform:none;max-height:none;margin:0">
    <header>
      <h2 id="demo-alert-dialog-title">Delete account?</h2>
      <p id="demo-alert-dialog-description">Your account, projects, and history will be permanently deleted. This action cannot be undone.</p>
    </header>
    <footer>
      <button type="button" class="btn" data-variant="outline" onclick="this.closest('dialog').close()">Cancel</button>
      <button type="button" class="btn" data-variant="destructive" onclick="this.closest('dialog').close()">Delete account</button>
    </footer>
  </div>
  </dialog>
</div>
...
<dialog id="demo-alert-dialog" class="alert-dialog" role="alertdialog" aria-labelledby="demo-alert-dialog-title" aria-describedby="demo-alert-dialog-description">
  <div>
    <header>
    ...
    <footer>
      ...
      <button class="btn" data-variant="destructive" onclick="this.closest('dialog').close()">Continue</button>
    </footer>
  </div>
</dialog>
```
...
<dl>
  ...
  <dd>Trigger button. A3S UI uses the native <code>HTMLDialogElement.showModal()</code> method.</dd>
...
Alert Dialog preserves the native `HTMLDialogElement` contract. React `useAlertDialog` and the equivalent Vue composable can call `showModal()` and `close(returnValue)`; `cancel` and `close` remain native DOM events.
...
<dialog id="alert-dialog-basic" class="alert-dialog" role="alertdialog" aria-labelledby="alert-dialog-basic-title" aria-describedby="alert-dialog-basic-description">
  <div>
    <header>
...
<dialog id="alert-dialog-media" class="alert-dialog" role="alertdialog" aria-labelledby="alert-dialog-media-title" aria-describedby="alert-dialog-media-description">
  <div>
    <header>
...
<dialog id="alert-dialog-destructive" class="alert-dialog" role="alertdialog" aria-labelledby="alert-dialog-destructive-title" aria-describedby="alert-dialog-destructive-description">
  <div>
    <header>
...
<dialog id="alert-dialog-small" class="alert-dialog" role="alertdialog" data-size="sm" aria-labelledby="alert-dialog-small-title" aria-describedby="alert-dialog-small-description">
  <div>
    <header>
...
<div dir="rtl">
  ...
  <dialog id="alert-dialog-rtl" class="alert-dialog" role="alertdialog" aria-labelledby="alert-dialog-rtl-title" aria-describedby="alert-dialog-rtl-description">
    <div>
      <header>
...
export function Example() {
  ...
  return (
    <AlertDialog ref={control.ref} aria-label="Alert Dialog"><div><header><h2>Component summary</h2></header><section>Component content</section><footer><button type="button">Continue</button></footer></div></AlertDialog>
  );
}
```

`useAlertDialog` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <AlertDialog ref="componentRef" aria-label="Alert Dialog"><div><header><h2>Component summary</h2></header><section>Component content</section><footer><button type="button">Continue</button></footer></div></AlertDialog>
</template>
```
