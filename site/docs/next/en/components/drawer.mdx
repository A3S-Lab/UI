...
export function Example() {
  ...
  return (
    <Drawer ref={control.ref} aria-label="Drawer"><div><header><h2>Component summary</h2></header><section>Component content</section><footer><button type="button">Continue</button></footer></div></Drawer>
  );
}
```

`useDrawer` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Drawer ref="componentRef" aria-label="Drawer"><div><header><h2>Component summary</h2></header><section>Component content</section><footer><button type="button">Continue</button></footer></div></Drawer>
</template>
```
