...
<Callout title="Presentation utility" icon="info">
  Spinner is a documented markup utility, not a controller-backed component. There is no `spinner.js` or `spinner.css`. A3S UI publishes a stable contract (`role="status"` + `animate-spin`) so HTML, React, and Vue examples share one selector for tests and adapters.
</Callout>
