...
进度条表示有明确起点与终点的任务完成程度，并在可能时提供可读的当前值。

<Preview class="w-full max-w-sm">
<div class="grid w-full gap-3" data-progress-primary-demo="zh">
  <p class="text-sm text-muted-foreground" data-progress-status aria-live="polite">已完成 66%</p>
  <div id="progress-demo" class="progress" role="progressbar" aria-label="加载进度" aria-valuenow="66" aria-valuemin="0" aria-valuemax="100" data-progress-bar>
    <span style="width: 66%"></span>
  </div>
  <div class="flex gap-2">
    <button type="button" class="btn" data-size="sm" data-progress-action="set-100" data-preview-onclick onclick="const demo=this.closest('[data-progress-primary-demo]'); const bar=demo.querySelector('[data-progress-bar]'); const fill=bar.querySelector('span'); const status=demo.querySelector('[data-progress-status]'); bar.setAttribute('aria-valuenow','100'); fill.style.width='100%'; status.textContent='已完成 100%';">完成</button>
    <button type="button" class="btn" data-size="sm" data-variant="outline" data-progress-action="set-0" data-preview-onclick onclick="const demo=this.closest('[data-progress-primary-demo]'); const bar=demo.querySelector('[data-progress-bar]'); const fill=bar.querySelector('span'); const status=demo.querySelector('[data-progress-status]'); bar.setAttribute('aria-valuenow','0'); fill.style.width='0%'; status.textContent='已完成 0%';">重置</button>
  </div>
</div>
</Preview>
