...
侧边栏承载产品的持久导航或工作区上下文，让用户在主要任务之间保持方向感。

<Preview>
<div class="a3s-sidebar-demo" data-sidebar-primary-demo="zh">
  ...
  <main>
    <div>
      ...
      <button type="button" class="btn" data-variant="outline" data-sidebar-toggle onclick="document.getElementById('demo-sidebar').toggle()">切换侧边栏</button>
    </div>
  </main>
</div>
</Preview>
...
<Callout title="支持模板宏" icon="code" action={{ label: "查看模板", href: "/templates#sidebar" }}>
...
`useSidebar` 订阅清单中声明的 DOM 事件，并且只调用清单中公开的方法。适配器不创建另一套框架专属状态。
