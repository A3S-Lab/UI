...
对话框在需要明确确认或短暂补充信息时将焦点限制在一个任务中，并在完成后返回原流程。

<Preview>
<div data-dialog-primary-demo="zh">
<button type="button" data-dialog-trigger onclick="document.getElementById('demo-dialog-edit-profile').showModal()" class="btn" data-variant="outline">打开对话框</button>
<dialog id="demo-dialog-edit-profile" class="dialog" aria-labelledby="demo-dialog-edit-profile-title" aria-describedby="demo-dialog-edit-profile-description" onclick="if (event.target === this) this.close()">
  <div class="sm:max-w-sm">
    <header>
      <h2 id="demo-dialog-edit-profile-title">编辑个人资料</h2>
      <p id="demo-dialog-edit-profile-description">修改个人资料，完成后保存更改。</p>
    </header>
    <section>
      <form class="grid gap-4">
        <div class="grid gap-3">
          <label class="label" for="demo-dialog-edit-profile-name">姓名</label>
          <input class="input" type="text" value="Pedro Duarte" id="demo-dialog-edit-profile-name" autofocus />
        </div>
        <div class="grid gap-3">
          <label class="label" for="demo-dialog-edit-profile-username">用户名</label>
          <input class="input" type="text" value="@peduarte" id="demo-dialog-edit-profile-username" />
        </div>
      </form>
    </section>
    <footer>
      <button class="btn" data-variant="outline" data-dialog-cancel onclick="this.closest('dialog').close()">取消</button>
      <button class="btn" data-dialog-save onclick="this.closest('dialog').close()">保存更改</button>
    </footer>
    <button type="button" class="btn" data-variant="ghost" data-size="icon-sm" aria-label="Close dialog" data-dialog-close onclick="this.closest('dialog').close()"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x-icon lucide-x"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg></button>
  </div>
</dialog>
</div>
</Preview>
...
<Callout title="支持模板宏" icon="code" action={{ label: "查看模板", href: "/templates#dialog" }}>
...
```html
<button type="button" onclick="document.getElementById('demo-dialog-edit-profile').showModal()" class="btn" data-variant="outline">打开对话框</button>
<dialog id="demo-dialog-edit-profile" class="dialog" aria-labelledby="demo-dialog-edit-profile-title" aria-describedby="demo-dialog-edit-profile-description" onclick="if (event.target === this) this.close()">
  <div class="sm:max-w-sm">
    <header>
      <h2 id="demo-dialog-edit-profile-title">编辑个人资料</h2>
    ...
    <footer>
      <button class="btn" data-variant="outline" onclick="this.closest('dialog').close()">取消</button>
      <button class="btn" onclick="this.closest('dialog').close()">保存更改</button>
    </footer>
...
`Dialog` 保留原生 `HTMLDialogElement` 契约。React 的 `useDialog` 与 Vue 的同名 composable 可调用 `showModal()` 和 `close(returnValue)`；`cancel` 与 `close` 仍是原生 DOM 事件。
...
```tsx
import { Dialog, useDialog } from "@a3s-lab/ui/react";
    ...
    >
      <div className="sm:max-w-sm">
        ...
        <footer>
          ...
          <button className="btn">保存更改</button>
        </footer>
        <button
          type="button"
          className="btn"
...
`useDialog` 订阅清单中声明的 DOM 事件，并且只调用清单中公开的方法。适配器不创建另一套框架专属状态。

## Vue

```vue
<script setup lang="ts">
import { Dialog, useDialog } from "@a3s-lab/ui/vue";
...
<template>
  ...
  >
    <div class="sm:max-w-sm">
      ...
      <footer>
        <button class="btn" data-variant="outline">取消</button>
