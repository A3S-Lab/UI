...
<Callout title="滚动条工具" icon="info">
  滚动区域是滚动条样式工具（`.scrollbar` / `.scrollbar-sm`），用于已通过 overflow 滚动的宿主容器。它不是滚动容器控制器，也没有专用 JavaScript。
</Callout>

滚动区域限制长内容的可视范围，同时保留内容顺序和可发现的滚动行为。

<Preview class="w-full max-w-42">
<div class="card h-72 p-0" data-scroll-area-primary-demo="zh">
  <div class="scrollbar h-full overflow-y-auto p-4" tabindex="0" data-scroll-area-viewport>
    <div class="space-y-4">
      <h4 class="text-sm leading-none font-medium" data-scroll-area-heading>标签</h4>
      <div class="text-sm">
...
区域滚动使用 A3S UI 滚动条工具类（`.scrollbar` / `.scrollbar-sm`）。

使用CDN或批量程序导入？请参见[安装页](/installation)。

  </Step>
  <Step title="添加滚动区域 HTML">
...
工具类增强了原生滚动的跨浏览器样式。它不封装内容、取代浏览器滚动行为或需要 JavaScript。需要键盘滚动的宿主应保持 overflow 元素可聚焦（`tabindex="0"` 或原生控件）。React/Vue 的 `ScrollArea` 导出在下一主版本迁移前仅作为兼容别名保留。
...
`useScrollArea` 订阅清单中声明的 DOM 事件，并且只调用清单中公开的方法。适配器不创建另一套框架专属状态。
