...
<Callout title="展示型工具" icon="info">
  Spinner 是已文档化的标记工具，不是带控制器的组件。没有 `spinner.js` 或 `spinner.css`。A3S UI 发布稳定约定（`role="status"` + `animate-spin`），使 HTML / React / Vue 示例共用同一选择器。框架侧 `Spinner` 导出在下一主版本移除适配器表面之前仅作为兼容别名保留。
</Callout>

<Preview class="w-full max-w-xs">
<article class="item bg-muted/50 border-transparent" data-spinner-primary-demo="zh" aria-busy="true">
  <figure><svg aria-label="加载中" role="status" class="animate-spin lucide lucide-loader-circle" data-spinner-status xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg></figure>
  <section> <h3 data-spinner-title>正在处理付款...</h3></section>
  <aside class="tabular-nums text-sm" data-spinner-amount>$100.00</aside>
</article>
</Preview>
...
用户启用减少动效后，完整 A3S UI 样式会停止旋转。请保留无障碍标签和相邻状态文案，让静止图标仍能清楚表达处理进度。
...
```tsx
import { Spinner } from "@a3s-lab/ui/react";
...
import { Spinner } from "@a3s-lab/ui/vue";
</script>
