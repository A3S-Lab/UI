...
空状态解释为什么没有内容，并在有可行下一步时提供单一、明确的恢复操作。

<Preview>
<section class="empty" data-empty-primary-demo="zh">
  <header>
    ...
    <h3>还没有项目</h3>
    <p>创建第一个项目，或导入已有仓库继续工作。</p>
  </header>
  <footer>
    <div class="flex gap-2">
      <button type="button" class="btn" data-empty-action="create" data-preview-onclick onclick="this.closest('[data-empty-primary-demo]').dataset.emptyStatus='create'">创建项目</button>
      <button type="button" class="btn" data-variant="outline" data-empty-action="import">导入项目</button>
    </div>
...
使用CDN或批量程序导入？请参见[安装页](/installation)。
  ...
  <Step title="添加空状态 HTML">

将 `class="empty"` 添加到语义容器。该组件设置常见子项的样式，例如 `header`、`figure`、标题、描述和 `footer`。
...
<section class="empty">
  ...
  <footer>
    <div class="input-group sm:w-3/4">
      <input type="search" aria-label="搜索页面" placeholder="Try searching for pages..." />
