...
<div class="grid w-full gap-3" data-toast-primary-demo="zh">
  <p class="text-sm text-muted-foreground" data-toast-status aria-live="polite">尚未显示提示</p>
  <div id="toaster-zh" class="toaster" data-toast-toaster></div>
  <button
    type="button"
    class="btn"
    ...
    onclick="const demo = this.closest('[data-toast-primary-demo]'); const toaster = demo.querySelector('[data-toast-toaster]'); const status = demo.querySelector('[data-toast-status]'); toaster.toast({ category: 'success', title: '操作成功', description: '更改已保存，可以继续当前任务。', duration: -1, cancel: { label: '关闭' } }); status.textContent = '成功提示已显示';"
  >显示成功提示</button>
</div>
</Preview>
