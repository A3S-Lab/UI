...
弹出框在触发器附近呈现少量补充内容或轻量操作，不中断页面主任务。

<Preview>
<div data-popover-primary-demo="zh" id="demo-popover" class="popover">
  <button id="demo-popover-trigger" data-popover-trigger type="button" aria-expanded="false" aria-controls="demo-popover-popover" class="btn" data-variant="outline">打开弹出框</button>
  <div id="demo-popover-popover" data-popover aria-hidden="true" class="w-72">
    ...
    <form class="grid gap-2" onsubmit="event.preventDefault()">
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-width">宽度</label>
        <input type="text" id="demo-popover-width" data-popover-field="width" value="100%" class="input col-span-2 h-8" autofocus />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-max-width">最大宽度</label>
        <input type="text" id="demo-popover-max-width" data-popover-field="max-width" value="300px" class="input col-span-2 h-8" />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-height">高度</label>
        <input type="text" id="demo-popover-height" data-popover-field="height" value="25px" class="input col-span-2 h-8" />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-max-height">最大高度</label>
        <input type="text" id="demo-popover-max-height" data-popover-field="max-height" value="none" class="input col-span-2 h-8" />
      </div>
    </form>
  </div>
</div>
...
<Callout title="支持模板宏" icon="code" action={{ label: "查看模板", href: "/templates#popover" }}>
...
<dl>
  ...
  <dd>
    ...
    <dl>
      ...
      <dd>Popover 内容。支持 <code>data-side="top|right|bottom|left|inline-start|inline-end"</code> 与 <code>data-align="start|center|end"</code>。默认使用 8px 侧边偏移和 8px 视口碰撞留白；可通过 <code>data-side-offset</code> 与 <code>data-collision-padding</code> 覆盖，或设置 <code>data-collision="none"</code> 保留静态 CSS 定位。运行时同样会定位以 <code>aria-hidden="false"</code> 条件挂载的 Popover。定位期间会发布 <code>--available-height</code>、<code>data-resolved-side</code>、<code>data-resolved-align</code> 与 <code>data-a3s-positioned="true"</code>。</dd>
    </dl>
  </dd>
</dl>
...
| API | 类型 | 说明 |
| --- | --- | --- |
| `popover.open(focus = true)` | 方法 | 打开浮层，并在存在 `[autofocus]` 时将焦点移入。 |
| `popover.close(focusOnTrigger = true)` | 方法 | 关闭浮层，并按需把焦点还给触发器。 |
| `popover.toggle(focus = true)` | 方法 | 根据当前 `aria-expanded` 状态切换。 |
| `popover.refresh()` | 方法 | 替换直接子触发器或内容后重新扫描。 |

React 的 `usePopover` 与 Vue 的同名 composable 通过 `call()` 暴露相同方法。控制器不管理表单数据、远程请求或业务权限。
...
`usePopover` 订阅清单中声明的 DOM 事件，并且只调用清单中公开的方法。适配器不创建另一套框架专属状态。
