...
Popover presents a small amount of supporting content or lightweight controls near its trigger without interrupting the page's primary task.

<Preview>
<div data-popover-primary-demo="en" id="demo-popover" class="popover">
  <button id="demo-popover-trigger" data-popover-trigger type="button" aria-expanded="false" aria-controls="demo-popover-popover" class="btn" data-variant="outline">Open popover</button>
  <div id="demo-popover-popover" data-popover aria-hidden="true" class="w-72">
    ...
    <form class="grid gap-2" onsubmit="event.preventDefault()">
      <div class="grid grid-cols-3 items-center gap-4">
        ...
        <input type="text" id="demo-popover-width" data-popover-field="width" defaultValue="100%" class="input col-span-2 h-8" autofocus />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-max-width">Max. width</label>
        <input type="text" id="demo-popover-max-width" data-popover-field="max-width" defaultValue="300px" class="input col-span-2 h-8" />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-height">Height</label>
        <input type="text" id="demo-popover-height" data-popover-field="height" defaultValue="25px" class="input col-span-2 h-8" />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="demo-popover-max-height">Max. height</label>
        <input type="text" id="demo-popover-max-height" data-popover-field="max-height" defaultValue="none" class="input col-span-2 h-8" />
      </div>
    </form>
  </div>
</div>
...
<dl>
  ...
  <dd>
    ...
    <dl>
      ...
      <dd>Popover content. Supports <code>data-side="top|right|bottom|left|inline-start|inline-end"</code> and <code>data-align="start|center|end"</code>. Open popovers use an 8-pixel side offset and 8-pixel viewport collision padding by default. Override them with <code>data-side-offset</code> and <code>data-collision-padding</code>, or set <code>data-collision="none"</code> to keep static CSS positioning. The runtime also positions popovers that are conditionally mounted with <code>aria-hidden="false"</code>. While positioned, it publishes <code>--available-height</code>, <code>data-resolved-side</code>, <code>data-resolved-align</code>, and <code>data-a3s-positioned="true"</code>.</dd>
    </dl>
  </dd>
</dl>
...
| --- | --- | --- |
| `popover.open(focus = true)` | Method | Opens the surface and focuses `[autofocus]` when present. |
| `popover.close(focusOnTrigger = true)` | Method | Closes the surface and optionally restores trigger focus. |
| `popover.toggle(focus = true)` | Method | Toggles from the current `aria-expanded` state. |
| `popover.refresh()` | Method | Rescans replaced direct trigger or content children. |

React `usePopover` and the equivalent Vue composable expose the same methods through `call()`. The controller does not own form data, remote requests, or product permissions.
  ...
  <div id="popover-form-popover" data-popover aria-hidden="true" class="w-72">
    ...
    <form class="grid gap-2" onsubmit="event.preventDefault()">
      <div class="grid grid-cols-3 items-center gap-4">
        ...
        <input type="text" id="popover-form-width" defaultValue="100%" class="input col-span-2 h-8" autoFocus />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="popover-form-max-width">Max. width</label>
        <input type="text" id="popover-form-max-width" defaultValue="300px" class="input col-span-2 h-8" />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="popover-form-height">Height</label>
        <input type="text" id="popover-form-height" defaultValue="25px" class="input col-span-2 h-8" />
      </div>
      <div class="grid grid-cols-3 items-center gap-4">
        <label for="popover-form-max-height">Max. height</label>
        <input type="text" id="popover-form-max-height" defaultValue="none" class="input col-span-2 h-8" />
      </div>
    </form>
  </div>
</div>
...
```tsx
import { Popover, usePopover } from "@a3s-lab/ui/react";

export function Example() {
  const control = usePopover();
  return (
    <Popover ref={control.ref} id="demo-popover">
      <button
        ...
        <form className="grid gap-2">
          <div className="grid grid-cols-3 items-center gap-4">
            ...
            <input
              ...
              defaultValue="100%"
              className="input col-span-2 h-8"
              autoFocus
            />
          </div>
          <div className="grid grid-cols-3 items-center gap-4">
            <label htmlFor="demo-popover-max-width">Max. width</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-3 items-center gap-4">
            <label htmlFor="demo-popover-max-height">Max. height</label>
            <input
              type="text"
...
`usePopover` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.

## Vue

```vue
<script setup lang="ts">
import { Popover, usePopover } from "@a3s-lab/ui/vue";
...
<template>
  <Popover ref="componentRef" id="demo-popover">
    <button
      id="demo-popover-trigger"
      type="button"
      aria-expanded="false"
