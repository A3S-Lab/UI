...
Dialog contains focus within a short task that needs explicit confirmation or additional input, then returns users to the originating flow.

<Preview>
<div data-dialog-primary-demo="en">
<button type="button" data-dialog-trigger onclick="document.getElementById('demo-dialog-edit-profile').showModal()" class="btn" data-variant="outline">Open Dialog</button>
    ...
    <section>
      <form class="grid gap-4">
        <div class="grid gap-3">
          ...
          <input class="input" type="text" defaultValue="Pedro Duarte" id="demo-dialog-edit-profile-name" autoFocus />
        </div>
        <div class="grid gap-3">
          <label class="label" for="demo-dialog-edit-profile-username">Username</label>
          <input class="input" type="text" defaultValue="@peduarte" id="demo-dialog-edit-profile-username" />
        </div>
      </form>
    </section>
    <footer>
      <button class="btn" data-variant="outline" data-dialog-cancel onclick="this.closest('dialog').close()">Cancel</button>
      <button class="btn" data-dialog-save onclick="this.closest('dialog').close()">Save changes</button>
    </footer>
...
Dialog preserves the native `HTMLDialogElement` contract. React `useDialog` and the equivalent Vue composable can call `showModal()` and `close(returnValue)`; `cancel` and `close` remain native DOM events.
      ...
      <section>
        <form class="grid gap-4">
          <div class="grid gap-3">
            ...
            <input class="input" id="dialog-rtl-name" name="name" defaultValue="Pedro Duarte" />
          </div>
          <div class="grid gap-3">
            <label class="label" for="dialog-rtl-username">اسم المستخدم</label>
            <input class="input" id="dialog-rtl-username" name="username" defaultValue="@peduarte" />
          </div>
        </form>
      </section>
      <footer>
...
`useDialog` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
