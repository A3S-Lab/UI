...
Select provides a styled single-choice list when the product needs a consistent popup or richer option content. Use Combobox when users need to search or create values.

<Preview title="Fruit choice">
  <div class="grid gap-3" data-select-primary-demo="en">
    <div id="select-demo" class="select" data-placeholder="Select a fruit">
      <button type="button" role="combobox" class="w-[180px]" id="select-demo-trigger" aria-label="Fruit" aria-haspopup="listbox" aria-autocomplete="none" aria-expanded="false" aria-controls="select-demo-listbox">
    ...
    <p data-select-status aria-live="polite">No fruit selected yet.</p>
  </div>
</Preview>
...
<div id="select-theme" class="select" data-placeholder="Theme">
  <button type="button" role="combobox" class="w-[180px]" id="select-theme-trigger" aria-label="Theme" aria-haspopup="listbox" aria-autocomplete="none" aria-expanded="false" aria-controls="select-theme-listbox">
...
<dl>
  ...
  <dd>
    ...
    <dl>
      ...
      <dd>Select-only combobox trigger. Give it a stable accessible name with a connected <code>&lt;label&gt;</code>, <code>aria-label</code>, or <code>aria-labelledby</code>; placeholder text is a value hint, not a label. Use <code>role="combobox"</code>, <code>aria-haspopup="listbox"</code>, <code>aria-autocomplete="none"</code>, <code>aria-expanded</code>, and <code>aria-controls</code>. The controller supplies stable option IDs for <code>aria-activedescendant</code>.</dd>
...
<div id="select-groups" class="select" data-placeholder="Select a fruit">
  <button type="button" role="combobox" class="w-[180px]" id="select-groups-trigger" aria-label="Fruit" aria-haspopup="listbox" aria-autocomplete="none" aria-expanded="false" aria-controls="select-groups-listbox">
    <span class="truncate">Select a fruit</span>
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down text-muted-foreground opacity-50 shrink-0"><path d="m6 9 6 6 6-6" /></svg>
  </button>
...
<div id="select-scrollable" class="select" data-placeholder="Select a timezone">
  <button type="button" role="combobox" class="w-[280px]" id="select-scrollable-trigger" aria-label="Timezone" aria-haspopup="listbox" aria-autocomplete="none" aria-expanded="false" aria-controls="select-scrollable-listbox">
...
<div id="select-disabled" class="select" data-placeholder="Select a fruit">
  <button type="button" role="combobox" class="w-[180px]" id="select-disabled-trigger" aria-label="Fruit" aria-haspopup="listbox" aria-autocomplete="none" aria-expanded="false" aria-controls="select-disabled-listbox" disabled>
    <span class="truncate">Select a fruit</span>
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down text-muted-foreground opacity-50 shrink-0"><path d="m6 9 6 6 6-6" /></svg>
  </button>
...
Disabled options stay visible for context but are skipped by pointer, arrow-key, Home, End, and typeahead navigation.

<Preview>
<div id="select-disabled-option" class="select" data-placeholder="Select a fruit">
  <button type="button" role="combobox" class="w-[180px]" id="select-disabled-option-trigger" aria-label="Fruit" aria-haspopup="listbox" aria-autocomplete="none" aria-expanded="false" aria-controls="select-disabled-option-listbox">
    <span class="truncate">Select a fruit</span>
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down text-muted-foreground opacity-50 shrink-0"><path d="m6 9 6 6 6-6" /></svg>
  </button>
  <div id="select-disabled-option-popover" data-popover aria-hidden="true">
    <div role="listbox" id="select-disabled-option-listbox" aria-orientation="vertical" aria-labelledby="select-disabled-option-trigger">
      <div role="option" data-value="apple">Apple</div>
      <div role="option" data-value="grapes" aria-disabled="true">Grapes — unavailable</div>
      <div role="option" data-value="pineapple">Pineapple</div>
    </div>
  </div>
...
Give every trigger a stable accessible name with a connected label, `aria-label`, or `aria-labelledby`; the placeholder and selected value do not replace the field label. Focus remains on the select-only combobox trigger while `aria-activedescendant` identifies the active option. Arrow keys move through enabled options, Home and End reach the first and last enabled option, printable text performs accent-insensitive typeahead, Enter or Space commits, and Escape closes without changing the value and restores trigger focus.
...
export function Example() {
  ...
  return (
    <Select ref={control.ref} id="select-theme" data-placeholder="Theme">
      <button
        type="button"
        role="combobox"
        className="w-[180px]"
...
`useSelect` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
...
<template>
  <Select ref="componentRef" id="select-theme" data-placeholder="Theme">
    <button
      type="button"
      role="combobox"
      class="w-[180px]"
