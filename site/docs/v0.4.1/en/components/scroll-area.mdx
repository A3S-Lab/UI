...
<Callout title="Scrollbar utility" icon="info">
  Scroll Area is a scrollbar styling utility (`.scrollbar` / `.scrollbar-sm`) for hosts that already scroll with overflow. It is not a scroll-container controller and has no dedicated JavaScript.
</Callout>

Scroll Area bounds long content while preserving its reading order and a discoverable, native-feeling scroll path.

<Preview class="w-full max-w-42">
<div class="card h-72 p-0" data-scroll-area-primary-demo="en">
  <div class="scrollbar h-full overflow-y-auto p-4" tabindex="0" data-scroll-area-viewport>
    <div class="space-y-4">
      <h4 class="text-sm leading-none font-medium" data-scroll-area-heading>Tags</h4>
      <div class="text-sm">
...
Scroll Area uses A3S UI scrollbar utility classes (`.scrollbar` / `.scrollbar-sm`).
...
The utility classes augment native scrolling for cross-browser styling. They do not wrap content, replace browser scroll behavior, or require JavaScript. Hosts that need keyboard scrolling should keep the overflow element focusable (`tabindex="0"` or a native control). React/Vue `ScrollArea` exports remain compatibility aliases until the next major migration.
...
```tsx
import { ScrollArea, useScrollArea } from "@a3s-lab/ui/react";

export function Example() {
  const control = useScrollArea();
  return (
    <ScrollArea
      ref={control.ref}
...
`useScrollArea` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.

## Vue

```vue
<script setup lang="ts">
import { ScrollArea, useScrollArea } from "@a3s-lab/ui/vue";
...
<template>
  <ScrollArea
    ref="componentRef"
    class="h-72 overflow-y-auto rounded-md border p-4"
  >
    Your scrollable content here.
  </ScrollArea>
</template>
