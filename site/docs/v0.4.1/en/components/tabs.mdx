...
Tabs switches between peer sections in the same page location, showing one panel at a time with an unambiguous current state.

<Preview class="max-w-[400px] w-full">
<div class="tabs w-full" id="demo-tabs-with-panels" data-tabs-primary-demo="en">
...
React `useTabs` and the equivalent Vue composable expose the same contract through `call("select", tab, focus)` and `call("refresh")`.
      ...
      >
        <div className="card">
          ...
          <section>
            <form className="grid gap-4">
              <div role="group" className="field">
                <label htmlFor="demo-tabs-password-current">
                  Current password
                </label>
                <input type="password" id="demo-tabs-password-current" />
              </div>
...
`useTabs` subscribes to manifest-declared DOM events and calls only public manifest methods. The adapter creates no second, framework-only state model.
