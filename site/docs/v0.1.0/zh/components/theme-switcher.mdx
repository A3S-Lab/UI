...
| 应用程序编程接口 | 类型 | 描述 |
| --- | --- | --- |
| `window.basecoat.theme.get()` | 方法 | 返回 `"dark"` 或 `"light"`。 |
| `window.basecoat.theme.set(mode)` | 方法 | 将模式设置为 `"dark"` 或 `"light"` 并将其存储在 `localStorage.themeMode` 中。 |
| `window.basecoat.theme.toggle()` | 方法 | 在深色和浅色模式之间切换。 |
